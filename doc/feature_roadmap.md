...
1. **Tier 1.1 Delete** — needed before everything else.
2. **Tier 1.2 Merge verts** — partially done, finish.
3. **Tier 1.5 Primitives** — execute `primitives_plan.md`.
4. **CC.4 Make polygon** + **CC.3 Mesh cleanup** — bundle as small commands.
5. **Tier 1.3 Poly Extrude** + **Tier 1.4 Edge Extrude** — pull out from bevel.
6. **Tier 2.3 Mirror** (without live symmetry).
7. **Tier 2.1 Add Loop** + **Tier 2.5 Smooth**.
8. **Tier 2.2 Bridge**.
9. **CC.1 Element Move** + **CC.2 Edge Slide**.
10. **Tier 2.4 Knife / Axis Slice**.

Stop here unless there's a concrete need for boolean / soft selection /
UV. Each Tier-3 item is a separate project.
