...
nfdresult_t NFD_Init(void) {
    ...
    // [vibe3d vendor patch, task 0431] libdbus defaults exit_on_disconnect=TRUE:
    // if the session bus drops mid-session (logout / dbus daemon restart) libdbus
    // would call _exit(1) and kill the whole application, losing unsaved work.
    // This connection is held process-wide for the app's entire lifetime, so
    // demote a disconnect to a soft failure instead of abrupt process death.
    // See third_party/nfde/PATCHES.md.
