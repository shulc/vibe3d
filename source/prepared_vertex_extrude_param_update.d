module prepared_vertex_extrude_param_update;

import core.atomic : atomicOp;
import document : Layer;
import mesh : Mesh;
import prepared_tool_effect : PreparedVertexExtrudeParamKind;
import tools.edit.vertex_extrude_tool : VertexExtrudeTool,
    PreparedVertexExtrudeParamImage;

struct PreparedVertexExtrudeParamToken { @disable this(this); private: ulong owner, generation; }
struct ValidatedVertexExtrudeParamToken { @disable this(this); private: ulong owner, generation; }
private shared ulong nextVertexExtrudeParamOwner;

final class PreparedVertexExtrudeParamUpdateOwner {
private:
    VertexExtrudeTool target_; Layer layer_; Mesh* source_;
    PreparedVertexExtrudeParamImage image_;
    immutable ulong owner_; ulong generation_;
    bool pending_, validated_, consumed_;
    PreparedVertexExtrudeParamToken prepared_;
    ValidatedVertexExtrudeParamToken validatedToken_;
public:
    @disable this();
    static PreparedVertexExtrudeParamUpdateOwner prepare(
            VertexExtrudeTool target, Layer layer) {
        ...
        auto owner = new PreparedVertexExtrudeParamUpdateOwner(target, layer);
    ...
    @property PreparedVertexExtrudeParamKind effectKind() const nothrow @nogc {
        return !image_.valid ? PreparedVertexExtrudeParamKind.None : image_.applies
            ? PreparedVertexExtrudeParamKind.Preview : PreparedVertexExtrudeParamKind.Noop;
    }
...
private:
    this(VertexExtrudeTool target, Layer layer) {
        target_ = target; layer_ = layer; source_ = &layer.meshRef();
        owner_ = atomicOp!"+="(nextVertexExtrudeParamOwner, 1UL);
    }
...
version(unittest) unittest {
    ...
    auto tool = new VertexExtrudeTool(() => &layer.meshRef(), &gpu, &mode,
        LitShader.init);
    ...
    assert(effect.accepted &&
        effect.kind == PreparedVertexExtrudeParamKind.Preview &&
    ...
    assert(layer.meshRef().vertices.length > oldVertices &&
        tool.preparedParamBuiltForTest() &&
        context.installTraceForTest() == [3,4,54,2,8]);

    auto zeroLayer = new Layer; zeroLayer.meshRef() = makeCube();
    zeroLayer.meshRef().syncSelection(); zeroLayer.meshRef().selectVertex(0);
    GpuMesh zeroGpu;
    auto zeroTool = new VertexExtrudeTool(() => &zeroLayer.meshRef(), &zeroGpu,
        &mode, LitShader.init);
    zeroTool.seedPreparedParamForTest(zeroLayer.meshRef(), true, 1.0f, 0.0f);
    ...
    assert(zero.accepted && zero.kind == PreparedVertexExtrudeParamKind.Preview &&
        zeroContext.validate()); zeroContext.install();
    assert(zeroLayer.meshRef().vertices.length == 8 &&
        !zeroTool.preparedParamBuiltForTest() &&
        zeroContext.installTraceForTest() == [3,4,54,2,8]);

    auto noopLayer = new Layer; noopLayer.meshRef() = makeCube();
    GpuMesh noopGpu;
    auto noopTool = new VertexExtrudeTool(() => &noopLayer.meshRef(), &noopGpu,
        &mode, LitShader.init);
    ...
    assert(noop.accepted && noop.kind == PreparedVertexExtrudeParamKind.Noop &&
        noopContext.validate()); noopContext.install();
    assert(noopContext.installTraceForTest() == [54,8]);

    auto staleLayer = new Layer; staleLayer.meshRef() = makeCube();
    staleLayer.meshRef().syncSelection(); staleLayer.meshRef().selectVertex(0);
    GpuMesh staleGpu;
    auto staleTool = new VertexExtrudeTool(() => &staleLayer.meshRef(), &staleGpu,
        &mode, LitShader.init);
    ...
    auto wrongTool = new VertexExtrudeTool(() => &wrongLayer.meshRef(), &staleGpu,
        &mode, LitShader.init);
