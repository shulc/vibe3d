...
struct Action {
    ...
    // For kind == script: each entry is an argstring line that
    ...
    // resolves true (a "popup face" that reflects the active option).
    // When no item is checked, falls back to the static `Button.label`.
    bool       dynamicLabel;
}
...
struct Button {
    ...
    // button doesn't react to hover or click. A side-panel convention
    // for "tool listed in this panel but not implemented".
    bool disabled;
    ...
    // Optional alternate behaviors keyed by modifier.
