module prepared_edge_bevel_param_update;

import core.atomic : atomicOp;
import document : Layer;
import mesh : Mesh;
import prepared_tool_effect : PreparedEdgeBevelParamKind;
import tools.edit.edge_bevel : EdgeBevelTool, PreparedEdgeBevelParamImage;

struct PreparedEdgeBevelParamToken {
    @disable this(this);
private: ulong owner, generation;
}
struct ValidatedEdgeBevelParamToken {
    @disable this(this);
private: ulong owner, generation;
}
private shared ulong nextEdgeBevelParamOwner;

final class PreparedEdgeBevelParamUpdateOwner {
private:
    EdgeBevelTool target_; Layer layer_; Mesh* source_;
    PreparedEdgeBevelParamImage image_;
    immutable ulong owner_; ulong generation_;
    bool pending_, validated_, consumed_;
    PreparedEdgeBevelParamToken prepared_;
    ValidatedEdgeBevelParamToken validatedToken_;
public:
    @disable this();
    static PreparedEdgeBevelParamUpdateOwner prepare(EdgeBevelTool target,
            Layer layer) {
        if (target is null || target.classinfo !is EdgeBevelTool.classinfo ||
            layer is null || !target.ownsPreparedLayer(layer)) return null;
        auto owner = new PreparedEdgeBevelParamUpdateOwner(target, layer);
    ...
    @property PreparedEdgeBevelParamKind effectKind() const nothrow @nogc {
        return !image_.valid ? PreparedEdgeBevelParamKind.None : image_.applies
            ? PreparedEdgeBevelParamKind.Preview : PreparedEdgeBevelParamKind.Noop;
    }
...
private:
    this(EdgeBevelTool target, Layer layer) {
        target_ = target; layer_ = layer; source_ = &layer.meshRef();
        owner_ = atomicOp!"+="(nextEdgeBevelParamOwner, 1UL);
    }
...
version(unittest) unittest {
    ...
    auto tool = new EdgeBevelTool(() => &layer.meshRef(), &gpu, &mode,
        LitShader.init);
    ...
    assert(effect.accepted && effect.kind == PreparedEdgeBevelParamKind.Preview &&
    ...
    assert(layer.meshRef().vertices.length > oldVertices &&
        tool.preparedParamInstalledForTest() &&
        context.installTraceForTest() == [3,4,46,2,8]);

    auto noopLayer = new Layer; noopLayer.meshRef() = makeCube();
    GpuMesh noopGpu;
    auto noopTool = new EdgeBevelTool(() => &noopLayer.meshRef(), &noopGpu,
        &mode, LitShader.init);
    ...
    auto wrongTool = new EdgeBevelTool(() => &wrongLayer.meshRef(), &staleGpu,
        &mode, LitShader.init);
