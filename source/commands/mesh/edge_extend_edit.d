module commands.mesh.edge_extend_edit;
...
/// Record-flavor command for an interactive Edge Extend session (see
/// EdgeExtendTool, Phase 4). The tool captures a full MeshSnapshot at the moment
/// extend topology is first built, mutates the mesh freely while the user drags
/// the gizmo and tweaks Tool Properties (intermediate revert+reapply cycles
/// within the tool itself), and on deactivation records this command holding
/// (pre-extend, post-extend) snapshots.
///
/// apply() = restore post; revert() = restore pre. A topology-creating tool
/// (new ridge verts + bridge faces) cannot use the vertex-position-delta
/// MeshVertexEdit undo, so it records a full before/after snapshot pair — one
/// undo step per session. Verbatim-style clone of MeshEdgeExtrudeEdit with the
/// name/label changed.
class MeshEdgeExtendEdit : Command, Operator {
    ...
    override string name()  const { return "mesh.edge_extend_edit"; }
    override string label() const {
        return editLabel.length ? editLabel : "Edge Extend";
    }

    // Change-scope metadata (Phase 4 §b). Extend appends ridge verts + bridge
    // faces and re-derives the ridge selection (no source-face reshape).
    ...
    void setSnapshots(MeshSnapshot before_, MeshSnapshot after_, string label_ = "Edge Extend") {
        this.before    = before_;
        this.after     = after_;
    ...
    // the extend kernel once inside a Mesh edit batch (see EdgeExtendTool.
    ...
    void setDelta(MeshEditDelta delta_, string label_ = "Edge Extend") {
        this.delta_    = delta_;
        this.useDelta_ = true;
        this.editLabel = label_;
    }
