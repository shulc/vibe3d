...
/// island (no boundary edges) are clean no-ops (the command refuses and
/// records nothing).
...
/// of the track that needed neither. ``Mesh.extrudeFacesByMask` (its SMOOTH arm)` was already armed
    ...
    bool evaluate(ref VectorStack vts) {
        ...
        if (undoRecorded()) {
            size_t nRedo;
            {
                ...
                nRedo = ed.extrudeFacesByMask(mask, shift_, true);
                ed.close();
            }
            return nRedo != 0;
        }
