...
class SelectByTag : Command {
    ...
    override bool apply() {
        ...
        if (name_.length > 0) {
            ...
            if (!found)
                throw new Exception(
                    "select.byTag: no surface named '" ~ name_ ~ "'");
        } else if (id_ >= 0) {
...
unittest {
    ...
    assert(msg.canFind("name"), "the reject must name the offending arg: " ~ msg);
}

// An unknown surface name throws rather than quietly selecting nothing — the
// two outcomes look identical on screen and only one of them is a typo.
unittest {
    import std.exception : collectExceptionMsg;
    import std.algorithm : canFind;
    auto m = taggedCube();
    ...
    auto msg = collectExceptionMsg(c.apply());
    assert(msg.canFind("Nope"), "the reject must quote the name: " ~ msg);
}
