module prepared_edge_extrude_param_update;

import core.atomic : atomicOp;
import document : Layer;
import mesh : Mesh;
import prepared_tool_effect : PreparedEdgeExtrudeParamKind;
import tools.edit.edge_extrude : EdgeExtrudeTool, PreparedEdgeExtrudeParamImage;

struct PreparedEdgeExtrudeParamToken {
    @disable this(this);
private: ulong owner, generation;
}
struct ValidatedEdgeExtrudeParamToken {
    @disable this(this);
private: ulong owner, generation;
}
private shared ulong nextEdgeExtrudeParamOwner;

final class PreparedEdgeExtrudeParamUpdateOwner {
private:
    EdgeExtrudeTool target_; Layer layer_; Mesh* source_;
    PreparedEdgeExtrudeParamImage image_;
    immutable ulong owner_; ulong generation_;
    bool pending_, validated_, consumed_;
    PreparedEdgeExtrudeParamToken prepared_;
    ValidatedEdgeExtrudeParamToken validatedToken_;
public:
    @disable this();
    static PreparedEdgeExtrudeParamUpdateOwner prepare(EdgeExtrudeTool target,
            Layer layer) {
        if (target is null || target.classinfo !is EdgeExtrudeTool.classinfo ||
            layer is null || !target.ownsPreparedLayer(layer)) return null;
        auto owner = new PreparedEdgeExtrudeParamUpdateOwner(target, layer);
    ...
    @property PreparedEdgeExtrudeParamKind effectKind() const nothrow @nogc {
        return !image_.valid ? PreparedEdgeExtrudeParamKind.None : image_.applies
            ? PreparedEdgeExtrudeParamKind.Preview : PreparedEdgeExtrudeParamKind.Noop;
    }
...
private:
    this(EdgeExtrudeTool target, Layer layer) {
        target_ = target; layer_ = layer; source_ = &layer.meshRef();
        owner_ = atomicOp!"+="(nextEdgeExtrudeParamOwner, 1UL);
    }
...
version(unittest) unittest {
    ...
    auto tool = new EdgeExtrudeTool(() => &layer.meshRef(), &gpu, &mode,
        LitShader.init);
    ...
    assert(effect.accepted && effect.kind == PreparedEdgeExtrudeParamKind.Preview &&
    ...
    assert(layer.meshRef().vertices.length > oldVertices &&
        tool.preparedParamBuiltForTest() &&
        context.installTraceForTest() == [3,4,47,2,8]);

    auto noopLayer = new Layer; noopLayer.meshRef() = makeCube();
    GpuMesh noopGpu;
    auto noopTool = new EdgeExtrudeTool(() => &noopLayer.meshRef(), &noopGpu,
        &mode, LitShader.init);
    ...
    auto wrongTool = new EdgeExtrudeTool(() => &wrongLayer.meshRef(), &staleGpu,
        &mode, LitShader.init);
