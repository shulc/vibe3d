...
//
// Wire schema follows the conventional `prim.cylinder` attributes.
//
    ...
    override bool onMouseMotion(ref const SDL_MouseMotionEvent e, ref VectorStack vts) {
        ...
        if (state == CylinderState.DrawingHeight) {
                                  ...
                                  hpOrigin, hpn, hit))
            {
                ...
                //   sizeOnAxis = halfH (size is stored as a per-axis radius).
...
private:
    ...
    // Commit guard mirror (deactivate() :320); compound, NOT `state != Idle`.
    // Category B preview-only cancel: scene mesh untouched until commit.
    public override bool hasUncommittedEdit() const {
        return (state == CylinderState.BaseSet)
            || (state >= CylinderState.DrawingHeight && currentHeight() > 1e-5f);
    }
    public override void cancelUncommittedEdit() { state = CylinderState.Idle; }
    ...
    public override void resyncSession() { state = CylinderState.Idle; }
    ...
    // prim.cylinder size is a half-extent, so the change in half-extent
