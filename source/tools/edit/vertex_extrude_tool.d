...
struct PreparedVertexExtrudeActivationImage {
    ...
    Vec3 anchor, baseAnchor, shiftAxis, widthAxis;
    ulong gizmoSelHash;
    void clear() nothrow @nogc { this = PreparedVertexExtrudeActivationImage.init; }
}
...
public:
    ...
    final PreparedVertexExtrudeActivationImage buildPreparedActivation(
            out Mesh* source) {
    ...
    final PreparedSessionActivateEffect prepareActivate(
        ...
        if (context is null) return PreparedSessionActivateEffect(
            preparedToolStateOwner, PreparedActivateKind.VertexExtrude, false);
        scope(failure) context.discard();
        auto owner = PreparedVertexExtrudeActivationOwner.prepare(this);
        bool ok = owner !is null && context.prepareVertexExtrudeActivation(owner) &&
...
private:
    ...
    private static void computePreparedGizmoFrame(ref Mesh source,
            ref PreparedVertexExtrudeActivationImage image) {
        ...
        image.widthAxis = (slen > 1e-6f) ? side * (1.0f/slen) : Vec3(1,0,0);
...
public:
    ...
    version(unittest) final void seedPreparedActivationForTest(ref Mesh oldMesh) {
        active = false; built = true; dragPart = 9; shift_ = 7; width_ = 8;
        gizmoValid = false; anchor = Vec3(1,2,3); baseAnchor = Vec3(4,5,6);
        shiftAxis = Vec3(7,8,9); widthAxis = Vec3(10,11,12);
    ...
    version(unittest) final bool preparedActivationDirtyForTest() const
            nothrow @nogc {
        return !active && built && dragPart == 9 && shift_ == 7 && width_ == 8 &&
    ...
    version(unittest) final bool preparedActivationForTest(size_t count,
        ...
        return active && !built && dragPart == -1 && shift_ == 0 && width_ == 0 &&
            ...
            baseAnchor == expectedBase && shiftAxis == expectedShift &&
            widthAxis == expectedWidth && gizmoSelHash == expectedHash &&
            dragLastMX == 14 && dragLastMY == 15 && dragBaseShift == 16 &&
            dragBaseWidth == 17 && cachedVp.view[0] == 18;
    }
    version(unittest) final PreparedVertexExtrudeActivationImage
