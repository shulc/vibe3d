...
private:
    ...
    void rebuildPreview() {
        previewMesh.clear();
        // During DrawingBase the axis-extent is 0; buildCone collapses to a
        // degenerate cone (apex coincident with the base centre). Render it
        // as the base ellipse only by short-circuiting via buildEllipseBase
        // — but to keep cone preview minimal we just call buildCone, which
        // produces a flat fan when halfA=0 (visible as the base ring).
