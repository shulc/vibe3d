...
// `mixin template MeshLoopSliceOps`, mixed into `struct Mesh`).
//
// Two receivers, chosen per entry by whether the entry MUTATES (plan §4.1):
//
//   * `ref MeshEditBatch ed` — `insertEdgeLoops` (both overloads) and
//     `insertEdgeLoopsMulti`. The batch is what defers the stamps of a cut
//     that adds a rail vertex per crossed face, rewrites the whole face
//     array and then resets the selection, into ONE at `close()`; and it is
//     what the Gap and Profile position writes below have to record INTO.
//   * `ref const(Mesh) m` — `loopSliceRingEdges`, `collectEdgeRing`,
//     `walkRingSide`, `railContinuation`. Every one of them only WALKS the
//     ring; `const` states that, and it is now enforced at the seam instead
//     of by a keyword a mixin body could have dropped at any time. Reaching
//     one of these from inside a kernel is spelled `f(ed.mesh, …)`, the
//     spelling Stage E1 settled on for `computeCollapsedFace_`.
//
// `ngonExitEdge`, `curvatureSplinePoint` and `capShellCycles` have NO
// receiver at all — they were `static` members and are plain module
// functions now. They have no receiver row in the census for that reason;
// the `static assert` tripwire at the foot of this file is what holds them
// (plan §2.6's convention, Stage D3 review forward note F-3).
//
// `struct EdgeRingEntry` MOVED TO MODULE SCOPE with them (§2.7). It stays
// `private`: `collectEdgeRing` returns `EdgeRingEntry[]` to four callers
// outside this module (`commands/mesh/loop_slice.d` ×2,
// `tools/slice/loop_slice_tool.d`, `tools/edit/topology_pen/tool.d` ×2) and
// every one of them binds the result with `auto` and reads only `.length`.
//
// WHAT `private` DOES HERE, STATED CORRECTLY (F1 review, M3). It hides the
// NAME and nothing else. `private` at MODULE scope in D restricts name
// LOOKUP; it is NOT an encapsulation boundary, and the compiler cannot tell
// "reads only `.length`" from "reads `.fi`". Probed with `dmd -o- -c`:
//
//     auto r = collectEdgeRing(m, 0, c); auto n = r.length;   // COMPILES
//     auto a = r[0].fi;  auto b = r[0].ngon;                  // COMPILES
//     typeof(r[0]) x;  x.fi = 3;  q ~= r[0];                  // COMPILES
//     EdgeRingEntry[] r;                                      // undefined identifier
//     import mesh_ops.loop_slice : EdgeRingEntry;             // not visible
//
// So "the build is green with `private`" would be green whatever those four
// callers read — the project's own "a check that cannot come out
// differently". The build proves only that nobody SPELLS the name outside.
// What actually holds the decision is the TEXT CENSUS row
// (`\nprivate struct EdgeRingEntry {` == 1) plus this comment; widening the
// type later is a decision somebody argues for, not something a green build
// licenses.
// An in-struct `alias EdgeRingEntry = …` putting the name back on `Mesh` is
// refused by the tripwire below, and that refusal is a mutation row
// (M-F1-ALIAS) rather than an argument.
//
// `capShellCycles` — the shared Cap Sections boundary-loop geometry (Loop
// Slice task 0252/0261 + Slice S8 task 0274), which lives here because Loop
// Slice is its primary owner. Its OTHER caller is `splitAlongCutLoop` in
// `mesh_ops/cut.d`. Between Stage E3 and Stage F1 that call was spelled
// `Mesh.capShellCycles(ed.faces, set)` — cut.d had already converted while
// this file was still a mixin, so the name was only reachable as a `static`
// member of the struct. F1 removes that member, so both cut.d call sites are
// BARE again in this same commit, and the compiler is what enforces it (a
// member beats a same-name UFCS free function only while the member exists).
// The census carries the absence pin: `Mesh.capShellCycles` appears nowhere
// under `source/`.
//
...
// package move or UFCS free-functions). Function bodies below are verbatim
// from the mixin template — Stage F1 changed receivers, four raw position
// writes (see `kLoopSliceEditScope` and the Gap/Profile blocks) and nothing
// else; the ordered comment-stripped diff of the two texts is empty apart
// from those.
// ---------------------------------------------------------------------------

/// The edit scope every caller of this family declares when it opens its
/// batch. It lives HERE, beside the kernels, and not spelled out at each of
/// the call sites, for the reason D2 gave for `kReduceEditScope`: N copies is
/// N chances to drift, and the one that drifts is the one that stops matching
/// the op-log's declared scope when Stage L9 turns this family's undo into a
/// delta (`MeshEditTracker.declare` is what ends up in `MeshEditDelta.scope_`).
///
/// `Geometry` = Points|Polygons: every cut appends a rail vertex per crossed
/// rail and replaces the whole face array. `Marks` because the tail
/// `setFaceMarksFrom` + `resetSelection` rewrite the mark words and clear
/// every selection. `Position` because the Gap (task 0253) and Profile
/// (task 0256) options MOVE vertices through `ed.setVertexPositions` — the
/// same third bit `kCutEditScope` carries, and for the same option.
enum uint kLoopSliceEditScope =
...
private int ngonExitEdge(uint n, int entryJ) {
...
private Vec3 curvatureSplinePoint(Vec3 p0, Vec3 p1, Vec3 p2, Vec3 p3,
...
private Vec3 railContinuation(ref const(Mesh) m, uint pivot, uint other) {
    Vec3  dir  = normalize(m.vertices[pivot] - m.vertices[other]);
    float best = 0.0f;                 // require a genuinely forward neighbour
    uint  bestV = uint.max;
    foreach (nb; m.verticesAroundVertex(pivot)) {
        if (nb == other) continue;
        Vec3  d  = normalize(m.vertices[nb] - m.vertices[pivot]);
        float al = dot(d, dir);
        if (al > best) { best = al; bestV = nb; }
    }
    if (bestV != uint.max) return m.vertices[bestV];
    return m.vertices[pivot] * 2.0f - m.vertices[other];   // reflect ⇒ linear tangent
}
...
private EdgeRingEntry[] walkRingSide(ref const(Mesh) m, uint seedEdge, uint startFace,
                                     out bool closed, bool ngon = false) {
    EdgeRingEntry[] result;
    closed = false;
    if (startFace >= m.faces.length) return result;
    if (m.faces[startFace].length != 4) return result;

    ulong seedKey = m.edgeKeyOf(seedEdge);
    int j0 = m.findEdgeInFace(startFace, seedKey);
    ...
    for (;;) {
        ...
        if (N == 4) {
            ...
            vis[curFi] = true;
            exitEi = m.edgeIndex(c, d);
            if (exitEi == ~0u) break;                   // no exit edge
            exitKey = m.edgeKeyOf(exitEi);
        } else {
...
int[] loopSliceRingEdges(ref const(Mesh) m, uint seedEdge) {
    if (seedEdge >= m.edges.length) return [];
    bool closed;
    auto ring = collectEdgeRing(m, seedEdge, closed);
    int[] res = [cast(int)seedEdge];
    foreach (ent; ring) {
        uint ei = m.edgeIndex(ent.c, ent.d);
...
EdgeRingEntry[] collectEdgeRing(ref const(Mesh) m, uint seedEdge, out bool closed,
                                bool ngon = false) {
    closed = false;
    if (seedEdge >= m.edges.length) return [];

    uint[2] incFaces; uint nFaces = 0;
    foreach (fi; m.facesAroundEdge(seedEdge))
    ...
    uint[] quadSides;
    foreach (i; 0 .. nFaces)
        if (m.faces[incFaces[i]].length == 4) quadSides ~= incFaces[i];
    if (quadSides.length == 0) return [];

    bool closedA;
    auto sideA = walkRingSide(m, seedEdge, quadSides[0], closedA, ngon);
    if (closedA) { closed = true; return sideA; }  // one pass hit closure

    if (quadSides.length == 1) return sideA;       // boundary / non-quad other side

    bool closedB;
    auto sideB = walkRingSide(m, seedEdge, quadSides[1], closedB, ngon);
...
bool insertEdgeLoops(ref MeshEditBatch ed, uint seedEdge, const(float)[] positions) {
    uint[] unused;
    return insertEdgeLoops(ed, seedEdge, positions, unused);
}
...
bool insertEdgeLoops(ref MeshEditBatch ed, uint seedEdge, const(float)[] positions,
    ...
    return insertEdgeLoopsMulti(ed, [seedEdge], positions, newFaceIndices);
}
...
bool insertEdgeLoopsMulti(ref MeshEditBatch ed, const(uint)[] seeds, const(float)[] positionsIn,
                          out uint[] newFaceIndices,
    ...
    if (!bandMode) {
        ...
        foreach (seed; seeds) {
            if (seed >= ed.edges.length) continue;
            bool closed;
            auto ring = collectEdgeRing(ed.mesh, seed, closed, ngon);
    ...
    Vec3 railNormal(uint va, uint vb) {
        uint ei = ed.edgeIndex(va, vb);
        if (ei == ~0u) return Vec3(0, 1, 0);
        Vec3 sum = Vec3(0, 0, 0);
        foreach (fi; ed.facesAroundEdge(ei)) sum = sum + ed.faceNormal(fi);
    ...
    uint[] getMids(uint va, uint vb, bool mirror = false) {
        ...
        uint[] mids;
        Vec3 va3 = ed.vertices[va], vb3 = ed.vertices[vb];
        if (curvature) {
            ...
            Vec3 p0 = railContinuation(ed.mesh, va, vb);
            Vec3 p3 = railContinuation(ed.mesh, vb, va);
            foreach (float t; positions) {
                float tt = mirror ? 1.0f - t : t;
                uint nv = ed.addVertex(curvatureSplinePoint(p0, va3, vb3, p3, tt, curveTension));
                mids ~= nv;
            ...
            foreach (float t; positions) {
                float tt = mirror ? 1.0f - t : t;
                uint nv = ed.addVertex(va3 + (vb3 - va3) * tt);
                mids ~= nv;
                if (carryUv) recordRailBlend(nv, va, vb, tt);
            }
        }
    ...
    uint[] railMids(uint va, uint vb, bool towardFirst, bool mirror = false) {
        ...
        if (rails[rp].midsVb is null) {
            ...
            Vec3 dir = normalize(ed.vertices[cvb] - ed.vertices[cva]);
            uint[] dup;
            foreach (v; rails[rp].midsVa) {
                uint nv = ed.addVertex(ed.vertices[v]);
                dup ~= nv;
    ...
    uint[][] newFaces;
    newFaces.reserve(ed.faces.length + rings.length * positions.length * 4);
    ...
    immutable size_t faceArrayEstimate =
        ed.faces.length + rings.length * positions.length * 4;
    uint[] newWord;
    ...
    void splitFace(uint fi) {
        ...
        foreach (i; 0 .. Pu)
            foreach (j; 0 .. Pv) {
                ...
                Vec3 pt = ed.vertices[A] * ((1.0f - u) * (1.0f - v))
                        + ed.vertices[B] * (u * (1.0f - v))
                        + ed.vertices[C] * (u * v)
                        + ed.vertices[D] * ((1.0f - u) * v);
                uint nv = ed.addVertex(pt);
                grid[i+1][j+1] = nv;
    ...
    void splitFaceTracked(uint fi) {
        ...
        splitFace(fi);
        immutable uint word = Mesh.faceAttrOr(ed.faceMarks, fi);
    ...
    if (bandMode) {
        auto bandChainsOut = bandWalk(ed.faces, bandFaces);
    ...
    if (!twoPass) {
        ...
        foreach (uint fi; 0 .. cast(uint)ed.faces.length) {
            if (fi in perFaceRings) splitFaceTracked(fi);
            else {
                newFaces ~= ed.faces[fi].dup;
                newWord  ~= Mesh.faceAttrOr(ed.faceMarks, fi);
                newSrc   ~= fi;
            }
        }
    } else {
    ...
    if (split && caps && splitSeams.length > 0) {
        ...
        uint capWord = ed.Marks.Hide;
        foreach (fi, _; perFaceRings) {
            capWord = Mesh.combineFaceMarksWords(capWord, Mesh.faceAttrOr(ed.faceMarks, fi));
            ...
            if ((capWord & ed.Marks.Subpatch) != 0 && (capWord & ed.Marks.Hide) == 0)
                break;
        }
    ...
    //
    // TWO OF THIS FAMILY'S FOUR RECORDED WRITES (task 1903 §2.5, §5.7). Until
    // Stage F1 these two lines were `vertices[pr[0]] = …` — RAW coordinate
    // writes, invisible to every mutation hook, so a recording batch around a
    // split-gap loop slice produced an op-log that named the added vertices
    // and the reshaped faces and said NOTHING about the seam pair it had just
    // pushed apart. §5.7's predicate could not see them either until Stage E3
    // widened it to admit one level of index nesting (`pr[0]` is one) — the
    // sibling pair in `mesh_ops/cut.d`'s gap block is what forced that, and
    // these four are the sites E3's re-measurement classified as F1's.
    //
    // Collect first, write once: `setVertexPositions` takes one op-log entry
    // and one deferred `commitChange` for the whole seam instead of one per
    // vertex, and its skip predicate is BIT identity, not `==`, so a write
    // that only flips a `-0.0` coordinate to `+0.0` still lands exactly as the
    // raw `v + d*half` did. Gathering before writing is bit-identical to the
    // sequential writes ONLY because each seam vertex belongs to exactly one
    // pair — `lo` is a distinct rail midpoint and `hi` is its sole duplicate,
    // minted by the Split hi-side `ed.addVertex(ed.vertices[v])` in this same
    // file (the `foreach (v; rails[rp].midsVa)` duplication loop; `getDup` is
    // `mesh_ops/cut.d`'s, not ours) — so no index
    // is written twice and no gathered read
    // can see another write's result. That is not left as an argument: the
    // Stage F1 differential runs the gap cells against the pre-conversion
    // sequential body and compares every coordinate's BITS.
    if (split && gap != 0.0f && splitSeams.length > 0) {
        immutable float half = gap * 0.5f;
        uint[] seamMoveIdx;
        Vec3[] seamMoveTo;
        seamMoveIdx.reserve(splitSeams.length * 2);
        seamMoveTo.reserve(splitSeams.length * 2);
        foreach (i, pr; splitSeams) {
            Vec3 d = splitSeamDirs[i];
            seamMoveIdx ~= pr[0];
            seamMoveTo  ~= ed.vertices[pr[0]] - d * half;   // lo → toward va
            seamMoveIdx ~= pr[1];
            seamMoveTo  ~= ed.vertices[pr[1]] + d * half;   // hi → toward vb
        }
        ed.setVertexPositions(seamMoveIdx, seamMoveTo);
    }
    ...
    //
    // THE OTHER TWO RECORDED WRITES (§5.7), same class and same reason as the
    // Gap pair above. The gather is bit-identical to the sequential writes for
    // the reason this block's own comment already states: a physical rail is
    // displaced ONCE however many faces reference it (the mid cache is per
    // rail), and `midsVb[i]` is that mid's Split duplicate — so the index set
    // is duplicate-free and no gathered read can see another write's result.
    // Measured against the pre-conversion body on the profile cells, bit for
    // bit.
    if (profileOn && profileDepth != 0.0f) {
        uint[] profMoveIdx;
        Vec3[] profMoveTo;
        // Reserve, as the Gap gather above does (F1 review, n1). The upper
        // bound is two entries per rail loop — `midsVa[i]` always, `midsVb[i]`
        // only under Split — and this runs on the 60 Hz preview path whenever
        // Profile is on, so the growth reallocations are per-frame.
        {
            size_t upper;
            foreach (ref r; rails)
                upper += r.midsVa.length * 2;
            profMoveIdx.reserve(upper);
            profMoveTo.reserve(upper);
        }
        foreach (ref r; rails) {
            foreach (i; 0 .. r.midsVa.length) {
                if (i >= profileHeights.length) break;
                Vec3 disp = r.normal * (profileHeights[i] * profileDepth);
                profMoveIdx ~= r.midsVa[i];
                profMoveTo  ~= ed.vertices[r.midsVa[i]] + disp;
                if (r.midsVb !is null && i < r.midsVb.length) {
                    profMoveIdx ~= r.midsVb[i];
                    profMoveTo  ~= ed.vertices[r.midsVb[i]] + disp;
                }
            }
        }
        ed.setVertexPositions(profMoveIdx, profMoveTo);
    }
    ...
    // on resize).
    assert(newWord.length == ed.faces.length,
           "insertEdgeLoopsMulti: newWord/newFaces length mismatch");
    ed.setFaceMarksFrom(newWord, ~ed.Marks.Select);
    ed.rebuildEdges();
    ed.buildLoops();
    ed.resetSelection();   // resizes + clears all selection; calls commitChange
    return true;
}
...
// in mesh_ops/cut.d, fed `ed.faces`) call it BARE, so the two produce
// byte-identical cap topology. It has NO receiver: it was a `static` member of
// `Mesh` until task 1903 Stage F1 and is a plain module function now, which is
// why cut.d's two call sites lost the `Mesh.` qualifier E3 had to give them.
// -----------------------------------------------------------------------
uint[][] capShellCycles(const(uint[])[] faceList, const bool[uint] set) {
...
// ---------------------------------------------------------------------------
// TRIPWIRE (task 1903 §2.7, the shape Stage E2 established and E3/E4 repeated).
// Every name this file owns must be a MODULE-level symbol and NOT a member of
// `struct Mesh`. Two ways it could become one again, and this refuses both:
//
//   * `mixin MeshLoopSliceOps;` coming back into `struct Mesh` — but the
//     template is gone, so that is a compile error on its own;
//   * a hand-written member, or an in-struct
//     `alias EdgeRingEntry = mesh_ops.loop_slice.EdgeRingEntry;`, put there
//     to keep an old `Mesh.X` spelling resolving. MEASURED at E2 and repeated
//     at E3/E4: an in-struct alias makes BOTH `Mesh.X` resolve AND
//     `__traits(hasMember, Mesh, "X")` answer `true`, so the alias and this
//     assert cannot both stand. That is the whole reason §2.7 forbids the
//     alias rather than merely preferring the move.
//
// This fires at `dub build` time, which is the half `commit_seam_census_test.d`
// cannot cover: a hand-written member leaves the census GREEN (it reads
// mesh.d's text for `mixin Mesh*Ops`, and a plain method is not one).
// The list includes the receiver-LESS helpers (`ngonExitEdge`,
// `curvatureSplinePoint`, `capShellCycles`) and the TYPE, because those are
// exactly the names with no receiver row in the census to hold them.
// ---------------------------------------------------------------------------
static foreach (n; ["EdgeRingEntry", "ngonExitEdge", "curvatureSplinePoint",
                    "railContinuation", "walkRingSide", "loopSliceRingEdges",
                    "collectEdgeRing", "insertEdgeLoops", "insertEdgeLoopsMulti",
                    "capShellCycles"])
    static assert(!__traits(hasMember, Mesh, n),
        "`Mesh." ~ n ~ "` is a MEMBER again. Task 1903 Stage F1 moved this "
      ~ "family out of `struct Mesh` into module-level free functions in "
      ~ "mesh_ops/loop_slice.d. A member BEATS a same-name UFCS free "
      ~ "function, so every call site would silently bind back to the member "
      ~ "and the free functions would go unreachable while every loop-slice "
      ~ "test stayed green. An in-struct `alias` counts: it makes "
      ~ "`hasMember` answer true, which is why plan §2.7 forbids it "
      ~ "(task 1903 §2.7, §4.5).");
...
// Task 1903 Stage F1: the two insert entries are free functions over
// `ref MeshEditBatch`, so these module unittests open their own UNRECORDED
// batch — the same shape `tests/unit/mesh_ops/loop_slice_test.d` uses, and the
// same reason: nothing here reads an op-log. The census counts these two opens
// and asserts they are exactly the unittest spelling, so a KERNEL that started
// opening one would be caught (cut.d's row has the same two-sided shape).
version (unittest)
...
unittest {
    // (b) — one seed per cube, N=2 loops each.
    {
        ...
        uint[] newFaceIndices;
        bool ok = sliceOnce!insertEdgeLoopsMulti(m, [eiA, eiB], [0.3f, 0.7f], newFaceIndices);
        ...
        bool okRef = sliceOnce!insertEdgeLoops(ref1, eiRef, [0.3f, 0.7f]);
    ...
    // includes both edge 0-1's and edge 2-3's midpoints).
    {
        ...
        uint[] newFaceIndices;
        bool ok = sliceOnce!insertEdgeLoopsMulti(m, [ei01, ei23], [0.5f], newFaceIndices);
        ...
        bool okSingle = sliceOnce!insertEdgeLoops(single, eiSingle, [0.5f]);
