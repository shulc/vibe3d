...
// `boundaryContourInset`).
//
// MODULE-LEVEL FREE FUNCTIONS over the edit seam since task 1903 Stage F2
// (was `mixin template MeshPolyBevelOps`, mixed into `struct Mesh`).
//
// THREE receiver classes, chosen per entry by what the entry TOUCHES
// (plan §4.1), and this file is the first that carries all three:
//
//   * `ref MeshEditBatch ed` — `insetFacesByMask`, `bevelFacesByMask`,
//     `spikeFacesByMask`. All three append vertices and faces, replace a
//     winding in place, resize the mark/material/part/set planes and end in
//     `commitChange(Geometry | Marks)`. The batch is what turns that tail —
//     plus every intermediate `addVertex` / `addFace` stamp — into ONE
//     deferred stamp at `close()`.
//   * `ref const(Mesh) m` — `cornerNormalAt` and `findGroupBoundaryContour`.
//     Both were `const` MEMBERS; nothing in a mixin body held that keyword,
//     and the const receiver states it at the seam instead. Reaching one from
//     inside a kernel is spelled `f(ed.mesh, …)`, the spelling Stage E1
//     settled on for `computeCollapsedFace_`.
//   * NO RECEIVER AT ALL — `insetCorner`, `insetCornerBisector`,
//     `maxSafeUniformInset`, `aveNormal`, `boundaryContourInset`. Every one
//     is pure arithmetic over a position ring or over two edge vectors: they
//     read no mesh state, and two of them (`aveNormal`,
//     `boundaryContourInset`) were already `static` members. They have no
//     receiver row in the census for that reason; the `static assert`
//     tripwire at the foot of this file is what holds them (plan §2.6's
//     convention, Stage D3 review forward note F-3).
//
// ALL SEVEN HELPERS STAY `private` AT MODULE SCOPE, and that decision is held
// by a TEXT CENSUS ROW, not by the build (plan §2.7 as corrected at the F1
// review; the same measurement `EdgeRingEntry` carries). `private` at module
// scope in D restricts NAME LOOKUP and nothing else: it is not an
// encapsulation boundary, so a green build here would be green whatever any
// outside caller did with these functions — it proves only that nobody
// SPELLS the names. What makes the decision visible is the census row that
// counts the `private` spelling. The reason for the decision is that
// `mesh.d`'s `public import mesh_ops.poly_bevel;` re-exports this module's
// PUBLIC names to every `import mesh;` client, and none of the seven has a
// caller outside this file: publishing them would open seven doors for
// nobody (the §2.6 rule, read in the direction it applies here).
//
// A NAME COLLISION THAT IS NOT ONE, WRITTEN DOWN SO STAGE H DOES NOT
// REDISCOVER IT: `mesh_ops/extrude.d` declares its OWN NESTED
// `Vec3 cornerNormalAt(uint v, int fi)` inside `extrudeEdgesByMask` — a
// different parameter order, a different law (it folds back to the face
// normal at a reflex corner) and a different owner. It never called this
// file's member and does not call this file's free function; the two names
// only ever coexisted because a mixin body's nested function shadows the
// enclosing struct's member. Stage H inherits the shadowing, not a caller.
//
// The six `unittest` blocks that used to live INSIDE the template body come
// with the functions and are ordinary module unittests now (plan §2.7). Five
...
//
// `mesh.buildRawMesh` — the raw fixture builder those five blocks share with
// `mesh.d`'s own T-S1 cases — is a §2.6 widening this stage pays for. It was
// `version (unittest) private` in `mesh`, reachable from here only because a
// mixin body is looked up in its INSTANTIATION scope; once these blocks are
// module unittests of THIS module that lookup is gone. It is widened (still
// `version (unittest)`, so the surface exists only in a unittest build) with
// a two-sided census row naming this file as its caller. Duplicating it here
// instead was declined for a measurable reason: a local copy would put an
// `m.vertices = verts;` back into this file and turn its §5.7 row from a
// clean `== 0` into a `kAllow` entry.
//
...
//
// Function bodies below are otherwise VERBATIM from the mixin template:
// Stage F2 changed the receivers, dedented by four, dropped the `Mesh.`
// qualifier the sixth unittest block needed while it was inside the template
// (`Mesh.boundaryContourInset` at three call sites — F2 REMOVES a qualifier
// where E3/E4 had to ADD one, because this file DECLARES the static it named
// rather than borrowing a sibling's), and nothing else. There is no
// `rewriteFaces` in this family and no raw position write: every new
// coordinate goes through `ed.addVertex`, which is why the §5.7 row here is a
// `== 0` STATEMENT rather than a retirement.
// ---------------------------------------------------------------------------

/// The edit scope every caller of this family declares when it opens its
/// batch. It lives HERE, beside the kernels, and is not spelled out at each
/// call site, for the reason D2 gave for `kReduceEditScope`: N copies is N
/// chances to drift, and the one that drifts is the one that stops matching
/// the op-log's declared scope when Stage K/L7 (bevel + inset) and Stage L2
/// (spikey) turn this family's undo into a delta.
///
/// ONE constant for all THREE entries, because all three declare the same
/// class and the file is where that sameness is stated once: `Geometry`
/// (= `Points | Polygons`) — every entry appends corner/apex vertices and
/// ring/fan faces and replaces the source winding in its own slot — and
/// `Marks`, because each rewrites subpatch bits and (spike) the face
/// selection. `Position` is ABSENT and that is the measurable difference
/// from `kLoopSliceEditScope`/`kCutEditScope`: no entry here moves an
/// EXISTING vertex, so this family consumes no `setVertexPos` at all.
///
/// The kernels' own `commitChange` keeps its literal spelling, exactly as
/// cut.d, cleanup.d and bevel_vertex.d do.
enum uint kPolyBevelEditScope = MeshEditScope.Geometry | MeshEditScope.Marks;
...
size_t insetFacesByMask(ref MeshEditBatch ed, const bool[] maskIn, float inset) {
    ...
    foreach (fi; 0 .. nFaces) {
        ...
        foreach (i; 0 .. N) origPos[i] = ed.vertices[origFaceVerts[i]];
        ...
        immutable bool srcSub  = ed.isFaceSubpatch(fi);
        immutable size_t ringStart = ed.faces.length;
        // Emit N ring quads bridging original boundary to inner boundary.
        foreach (i; 0 .. N) {
            const int next = (i + 1) % N;
            ed.addFace([origFaceVerts[i], origFaceVerts[next],
                     newVerts[next],   newVerts[i]]);
        }
        // Ring quads inherit Subpatch from the inset source face.
        ed.resizeSubpatch();
        foreach (rfi; ringStart .. ed.faces.length) ed.setFaceSubpatch(rfi, srcSub);
        ++processed;
    }
...
private Vec3 cornerNormalAt(ref const(Mesh) m, uint fi, uint v) {
    ...
    const Vec3 prevV = m.vertices[f[(k + N - 1) % N]];
    const Vec3 curV  = m.vertices[f[k]];
    const Vec3 nextV = m.vertices[f[(k + 1) % N]];
...
private Vec3 aveNormal(Vec3 nSum, uint count) {
...
private bool findGroupBoundaryContour(ref const(Mesh) m, uint v, const bool[] mask,
        bool[ulong] internalEdgeSet, out uint eA, out uint eB) {
    bool foundA = false, foundB = false;
    foreach (fi; m.facesAroundVertex(v)) {
        if (fi >= mask.length || !mask[fi]) continue;
        const uint[] f = m.faces[fi];
...
private bool boundaryContourInset(Vec3 eaVec, Vec3 ebVec,
...
unittest { // boundaryContourInset degeneracy gate (task 0467 reviewer NIT):
    ...
    assert(!boundaryContourInset(Vec3(1,0,0), Vec3(0,1,0), Vec3(0,2,0), 0.1f, res),
        "e_b ∥ aveN must fall to the fallback (degenerate U)");
    // Anti-parallel projection (G1-style D→0): D sums to ~zero, |D·U| gate.
    assert(!boundaryContourInset(Vec3(1,0,0), Vec3(-1,0,0), Vec3(0,0,1), 0.1f, res),
    ...
    assert(boundaryContourInset(Vec3(1,0,0), Vec3(0,1,0), Vec3(0,0,1), 0.1f, res),
...
/// defense.
size_t bevelFacesByMask(ref MeshEditBatch ed, const bool[] maskIn, float inset, float shift,
    ...
    const size_t nFaces = ed.faces.length;
    ...
    if (group) {
        ...
        foreach (fi; 0 .. nFaces) {
            if (fi >= mask.length || !mask[fi]) continue;
            foreach (v; ed.faces[fi]) {
                immutable Vec3 cn = cornerNormalAt(ed.mesh, cast(uint)fi, v);
        ...
        foreach (v, nSum; normalSum) {
            ...
            foreach (ei; ed.edgesAroundVertex(v)) {
                immutable uint w = ed.edgeOtherVertex(ei, v);
            ...
            if (internalCnt == 1) {
                ...
                uint eA, eB;
                Vec3 offset;
                if (findGroupBoundaryContour(ed.mesh, v, mask, internalEdgeSet, eA, eB) &&
                    boundaryContourInset(ed.vertices[eA] - ed.vertices[v],
                                          ed.vertices[eB] - ed.vertices[v], aveN, inset, offset)) {
                    sharedCornerPos[v] = ed.vertices[v] + sSum + offset;
                } else {
                    sharedCornerPos[v] = ed.vertices[v] + sSum +
                        safeNormalize(ed.vertices[lastInternalOther] - ed.vertices[v]) * inset;
                }
            } else if (!anyBoundary) {
    ...
    foreach (fi; 0 .. nFaces) {
        ...
        foreach (i; 0 .. Nc) origPos[i] = ed.vertices[origFaceVerts[i]];
        ...
        foreach (i; 0 .. Nc) {
            ...
            if (shP !is null) {
                ...
                if (auto p = origV in sharedVertIdxByLevel[Nseg]) finalVerts[i] = *p;
                    ...
                    immutable uint nv = ed.addVertex(finalPos[i]);
                ...
                // law.
                immutable Vec3 cn = cornerNormalAt(ed.mesh, cast(uint)fi, origV);
                Vec3 bcOffset;
                ...
                finalVerts[i] = ed.addVertex(finalPos[i]);
                noteRing(finalVerts[i], i, Nseg);
            }
        }
            ...
            foreach (i; 0 .. Nc) {
                ...
                if (group && (origV in sharedCornerPos) !is null) {
                    if (auto p = origV in sharedVertIdxByLevel[t]) ring[i] = *p;
                        ...
                        immutable uint nv = ed.addVertex(origPos[i] + (finalPos[i] - origPos[i]) * f);
                        sharedVertIdxByLevel[t][origV] = nv;
                        ring[i] = nv;
                    }
                } else {
                    ring[i] = ed.addVertex(origPos[i] + (finalPos[i] - origPos[i]) * f);
                }
                noteRing(ring[i], i, t);
            }
            ringVerts[t] = ring;
        ...
        if (doSquare) {
            ...
            foreach (i; 0 .. Nc) {
                ...
                splitToNext[i] = ed.addVertex(origPos[i] + dirNext * splitStep);
                splitToPrev[i] = ed.addVertex(origPos[i] + dirPrev * splitStep);
        ...
        foreach (i; 0 .. Nc) {
            ...
            if (doSquare) {
                ...
                ed.addFace([pointAtI, pointAtNext, ringVerts[1][next], ringVerts[1][i]]);
                foreach (t; 1 .. Nseg)
                    ed.addFace([ringVerts[t][i],     ringVerts[t][next],
        ...
        if (doSquare) {
            ...
            foreach (i; 0 .. Nc)
                if (!isRidgeCorner[i])
                    ed.addFace([origFaceVerts[i], splitToNext[i],
        ...
        immutable uint  srcMat  = fi < ed.faceMaterial.length ? ed.faceMaterial[fi] : 0u;
        immutable uint  srcPart = fi < ed.facePart.length     ? ed.facePart[fi]     : 0u;
        immutable ulong srcSet  = fi < ed.faceSetMask.length  ? ed.faceSetMask[fi]  : 0UL;
        if (ed.faceMaterial.length < ed.faces.length) ed.faceMaterial.length = ed.faces.length;
        if (ed.facePart.length     < ed.faces.length) ed.facePart.length     = ed.faces.length;
        if (ed.faceSetMask.length  < ed.faces.length) ed.faceSetMask.length  = ed.faces.length;
        foreach (rfi; ringStart .. ed.faces.length) {
            ed.faceMaterial[rfi] = srcMat;
            ed.facePart[rfi]     = srcPart;
            ed.faceSetMask[rfi]  = srcSet;
        }
        // Every face this source face appended reads its island (task 0697).
        if (remapUv) {
            faceSrcOf.length = ed.faces.length;
            foreach (rfi; ringStart .. ed.faces.length) faceSrcOf[rfi] = cast(uint)fi;
        }
        ++processed;
    }
    if (processed == 0) return 0;
    ...
    if (remapUv) {
        ...
        ed.declareCornerProvenance(
            rw.carried(ed.faces.range, srcOfCorner, vertBlend, gens));
    }
    if (anyClamped || group) {
...
// Task 1903 Stage F2: the three entries are free functions over
// `ref MeshEditBatch`, so the five MUTATING module unittests below open their
// own UNRECORDED batch through this one helper — the same shape F1's
// `sliceOnce` has in loop_slice.d, and the same reason: nothing in these
// blocks reads an op-log, so a RECORDING batch would build one for no reader
// and `close()` would drop it. GENERIC IN THE RETURN TYPE (`auto`), which is
// where it differs from `sliceOnce`: these kernels answer `size_t`, not
// `bool`. `auto ref Args` because a caller may pass an lvalue array literal.
//
// The census counts this open TWO-SIDED — the total must be exactly one
// unrecorded and zero recording, AND the one must be this exact spelling — so
// a KERNEL that started opening its own batch is caught either by the count
// or by the shape.
version (unittest)
private auto bevelOnce(alias kernel, Args...)(ref Mesh m, auto ref Args args) {
...
unittest { // bevelFacesByMask: GROUP accumulator on ASYMMETRIC geometry —
    ...
    auto m = buildRawMesh(
        ...
        [[0u,1,2,3], [1u,0,4,5]]);
    bool[] mask = [true, true];
    size_t n = bevelOnce!bevelFacesByMask(m, mask, 0.1f, 0.1f, true, 0);
    assert(n == 2);
...
unittest { // bevelFacesByMask: WARPED single quad, ISOLATED-face inset law —
    ...
    size_t nb = bevelOnce!bevelFacesByMask(m, [true], inset, shift, false, 0);
    assert(nb == 1);
...
unittest { // bevelFacesByMask: FLAT quad stays on the OLD whole-face law
    ...
    foreach (i; 0 .. 4) {
        ...
        expOld[i] = insetCorner(origPos, cast(int)i, n, inset) + n * shift;
    }
    assert(bevelOnce!bevelFacesByMask(m, [true], inset, shift, false, 0) == 1);
    foreach (i; 0 .. 4) {
        bool exact = false;
...
size_t spikeFacesByMask(ref MeshEditBatch ed, const bool[] maskIn, float amount) {
    const mask = ed.maskMinusHiddenFaces(maskIn);  // §3.3 backstop (task 0613) — see maskMinusHidden* in mesh.d
    size_t processed = 0;
    const size_t nFaces = ed.faces.length; // snapshot before appending fan tris
    ...
    foreach (fi; 0 .. nFaces) {
        ...
        const Vec3 c = ed.faceCentroid(cast(uint)fi);
        const Vec3 n = ed.faceNormal(cast(uint)fi);

        // Perimeter = sum of edge lengths around the face ring.
        float perimeter = 0f;
        foreach (i; 0 .. N) {
            Vec3  a  = ed.vertices[origFaceVerts[i]];
            Vec3  b  = ed.vertices[origFaceVerts[(i + 1) % N]];
        ...
        uint apex = ed.addVertex(c + n * disp);
    ...
    const size_t origMatLen  = ed.faceMaterial.length;
    const size_t origPartLen = ed.facePart.length;
    const size_t origSetLen  = ed.faceSetMask.length;   // task 1060, Stage 5c
    ed.resizeSubpatch();               // grows faceMarks to faces.length
    ed.faceMaterial.length = ed.faces.length;
    ed.facePart.length     = ed.faces.length;
    ed.faceSetMask.length  = ed.faces.length;
    foreach (k; 0 .. appendedFi.length) {
        const uint newFi = appendedFi[k];
        const uint srcFi  = fanSrc[k];
        ed.faceMaterial[newFi] = (srcFi < origMatLen  ? ed.faceMaterial[srcFi] : 0u);
        ed.facePart[newFi]     = (srcFi < origPartLen ? ed.facePart[srcFi]     : 0u);
        ed.faceSetMask[newFi]  = (srcFi < origSetLen  ? ed.faceSetMask[srcFi]  : 0UL);
        ed.setFaceSubpatch(newFi, ed.isFaceSubpatch(srcFi));
    }
    ...
    if (carryUv) {
        ...
        srcFaceOfNewFace.length = ed.faces.length;
    ...
    ed.rebuildEdges();
    ed.buildLoops();
    ed.syncSelection();  // grows faceSelectionOrder et al. to faces.length

    // D3: select all appended fan tris (slot fi stays selected via in-place).
    foreach (newFi; appendedFi) ed.selectFace(cast(int)newFi);
...
// ---------------------------------------------------------------------------
// TRIPWIRE — none of this family may be a MEMBER of `Mesh` again.
//
// A member BEATS a same-name UFCS free function, so re-adding one (by hand,
// by a reinstated `mixin MeshPolyBevelOps;`, or by an in-struct `alias`)
// would silently rebind every call site to the member and leave the free
// functions unreachable while every poly-bevel test stayed green. That is the
// failure plan Revision 2 caveat 1 describes, and it is invisible to
// `commit_seam_census_test.d`, which reads mesh.d's text for
// `mixin Mesh*Ops` — a hand-written method is not one.
//
// An in-struct `alias X = mesh_ops.poly_bevel.X;` counts: MEASURED at E2 and
// repeated at E3/E4/F1, an alias makes BOTH `Mesh.X` resolve AND
// `__traits(hasMember, Mesh, "X")` answer `true`, so the alias and this
// assert cannot both stand. That is why plan §2.7 forbids the alias rather
// than merely preferring the move.
//
// The list carries the five receiver-LESS helpers as well
// (`insetCorner`, `insetCornerBisector`, `maxSafeUniformInset`, `aveNormal`,
// `boundaryContourInset`), because those are exactly the names with no
// receiver row in the census to hold them.
// ---------------------------------------------------------------------------
static foreach (n; ["insetCorner", "insetCornerBisector", "insetFacesByMask",
                    "maxSafeUniformInset", "cornerNormalAt", "aveNormal",
                    "findGroupBoundaryContour", "boundaryContourInset",
                    "bevelFacesByMask", "spikeFacesByMask"])
    static assert(!__traits(hasMember, Mesh, n),
        "`Mesh." ~ n ~ "` is a MEMBER again. Task 1903 Stage F2 moved this "
      ~ "family out of `struct Mesh` into module-level free functions in "
      ~ "mesh_ops/poly_bevel.d. A member BEATS a same-name UFCS free "
      ~ "function, so every call site would silently bind back to the member "
      ~ "and the free functions would go unreachable while every poly-bevel, "
      ~ "poly-inset and spikey test stayed green. An in-struct `alias` "
      ~ "counts: it makes `hasMember` answer true, which is why plan §2.7 "
      ~ "forbids it (task 1903 §2.7, §4.5).");
