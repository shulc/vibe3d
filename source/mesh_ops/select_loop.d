...
// (selectLoopEdges and its three branch helpers), the vertex walk
// (selectLoopVertices) and the face band walk (selectLoopFaces), plus the
// frozen head-restart oracles the differential test measures the forward-only
// scans against.
//
// Split out of mesh.d as `mixin template MeshSelectLoopOps` (task 0717, audit
// 0678 §2B-M3 item 2); converted to module-level FREE FUNCTIONS over
// `ref const(Mesh) m` by task 1903 Stage C (`doc/mesh_edit_seam_plan.md` §4).
// Nothing in this family mutates: every entry was a `const` member, so it
// takes the mesh by `ref const` and opens no `MeshEditBatch`. It is the
// pure-mechanics rehearsal for the twelve mutating families that follow, and
// the mixin it replaced was deleted from `struct Mesh` in the SAME commit — a
// member reachable on the receiver BEATS a same-name UFCS free function, so a
// surviving `mixin MeshSelectLoopOps;` would silently keep answering every
// call and the conversion would change nothing (plan Revision 2 caveat 1).
// `tests/unit/commit_seam_census_test.d` holds that as a check.
//
// HOW THE CALL SITES REACH THESE. `source/mesh.d` carries
// `public import mesh_ops.select_loop;`, so every `import mesh;` re-exports
// them and `mesh.selectLoopEdges(seed)` keeps resolving through UFCS with the
// spelling it always had.
//
// THE ONE SPELLING THAT DOES NOT SURVIVE: a POINTER receiver. UFCS does not
// auto-dereference, so `meshPtr.selectLoopEdges(seed)` — which compiled while
// these were members — is now `Error: no property 'selectLoopEdges' for
// 'mesh' of type 'Mesh*'`. Those sites spell the deref: `(*mesh).…`. Every
// mutating family below has the same seam, and commands hold `Mesh* mesh`.
//
// The receiver is named `m` in every function on purpose (plan §4.3 step 3):
// the nested walkers capture the enclosing parameter exactly as they captured
// the enclosing `this`, so a stable name means they need only the same `m.`
// prefix and never an extra thread-through parameter.
// ---------------------------------------------------------------------------
import mesh;
import math;
...
bool isEdgeBorder(ref const(Mesh) m, uint ei) {
    uint n = 0;
...
bool isVertexBorder(ref const(Mesh) m, uint vi) {
    foreach (ei; m.edgesAroundVertex(vi))
        if (m.isEdgeBorder(ei)) return true;
    return false;
}

/// Count of border edges (`isEdgeBorder`) incident to vertex `vi`.
uint borderEdgeCountAtVertex(ref const(Mesh) m, uint vi) {
    uint n = 0;
    foreach (ei; m.edgesAroundVertex(vi))
        if (m.isEdgeBorder(ei)) ++n;
    return n;
}
...
private uint triangleOtherEdgeAt(ref const(Mesh) m, const(uint)[] tri, uint pivot, ulong excludeKey) {
    foreach (j; 0 .. 3) {
        uint va = tri[j], vb = tri[(j + 1) % 3];
        if (va != pivot && vb != pivot) continue;
        ulong k = edgeKey(va, vb);
        if (k == excludeKey) continue;
        uint ei = m.edgeIndexByKey(k);
        if (ei != ~0u) return ei;
    }
    return ~0u;
}
...
int[] selectLoopDirectionEdges(ref const(Mesh) m, uint seedEdge, uint startFace) {
    if (startFace >= m.faces.length) return [];
    const sfv0 = m.faces[startFace];
    if (sfv0.length < 3) return [];
    int si = m.findEdgeInFace(startFace, m.edgeKeyOf(seedEdge));
    if (si < 0) return [];
    uint pivot0 = sfv0[(cast(uint)si + 1) % sfv0.length];

    uint vcount = m.vertexValence(pivot0);
    if ((vcount & 1) != 0) return [];                                // gate 1
    if (vcount >= 4 && m.borderEdgeCountAtVertex(pivot0) > 2) return [];// gate 2
    if (m.isVertexBorder(pivot0)) return [];                          // gate 3

    if (sfv0.length == 3) {
        uint ei = m.triangleOtherEdgeAt(sfv0, pivot0, m.edgeKeyOf(seedEdge));
    ...
    int[] res;
    bool[ulong] vis; vis[m.edgeKeyOf(seedEdge)] = true;
    while (true) {
        const face = m.faces[curFace];
        ...
        uint sei = m.edgeIndex(b, c);
        if (sei == ~0u) break;
        int nf = m.adjacentFaceThrough(sei, cast(uint)curFace);
        if (nf < 0) break; // open boundary — natural terminating chain
        const nface = m.faces[nf];
        if (nface.length == 3) {
            uint ei = m.triangleOtherEdgeAt(nface, b, edgeKey(b, c));
...
int[] selectLoopFallbackFace(ref const(Mesh) m, const(uint)[] incFaces) {
    uint best = incFaces[0];
    foreach (fi; incFaces[1 .. $])
        if (m.faces[fi].length > m.faces[best].length) best = fi;
    const fv = m.faces[best];
    int[] result;
    bool[ulong] seen;
...
int[] selectLoopBorderChain(ref const(Mesh) m, uint seedEdge) {
    uint u = m.edges[seedEdge][0], v = m.edges[seedEdge][1];
    ...
    ChainResult chainFrom(uint from) {
        ...
        while (true) {
            ...
            foreach (ei; m.edgesAroundVertex(cur)) {
                ...
                ulong k = m.edgeKeyOf(ei);
                if (k in vis) continue;
                if (!m.isEdgeBorder(ei)) continue;
                next = cast(int)ei;
                break;
            }
            if (next < 0) return ChainResult(res, false);
            vis[m.edgeKeyOf(cast(uint)next)] = true;
            res ~= next;
            uint a = m.edges[next][0], b = m.edges[next][1];
            uint far = (a == cur) ? b : a;
    ...
    bool[ulong] seen;
    foreach (ei; result) seen[m.edgeKeyOf(cast(uint)ei)] = true;
    foreach (ei; dirU.edges) {
        ulong k = m.edgeKeyOf(cast(uint)ei);
...
int[] selectLoopEdges(ref const(Mesh) m, uint seedEdge) {
    if (seedEdge >= m.edges.length) return [];

    uint[] incFaces;
    foreach (fi; m.facesAroundEdge(seedEdge)) incFaces ~= fi;

    if (incFaces.length == 0) return [cast(int)seedEdge]; // stray/degenerate edge
    if (incFaces.length == 1) return m.selectLoopBorderChain(seedEdge); // rule 7

    bool anyHops = false;
    int[][] dirResults = new int[][](incFaces.length);
    foreach (i, fi; incFaces) {
        dirResults[i] = m.selectLoopDirectionEdges(seedEdge, fi);
        if (dirResults[i].length > 0) anyHops = true;
    }

    if (!anyHops) return m.selectLoopFallbackFace(incFaces); // rules 4 & 6
...
private void buildLoopIncidence(ref const(Mesh) m, out uint[][] vertEdges, out uint[][] edgeFaces,
                                out uint[][] vertFaces,
                                bool wantVertexStars = true) {
    if (wantVertexStars) {
        vertEdges = new uint[][](m.vertices.length);
        foreach (ei; 0 .. m.edges.length) {
            vertEdges[m.edges[ei][0]] ~= cast(uint)ei;
            vertEdges[m.edges[ei][1]] ~= cast(uint)ei;
        }
        vertFaces = new uint[][](m.vertices.length);
    }
    edgeFaces = new uint[][](m.edges.length);
    foreach (fi; 0 .. m.faces.length) {
        const f = m.faces[fi];
        if (wantVertexStars)
            foreach (fv; f)
                if (fv < m.vertices.length) vertFaces[fv] ~= cast(uint)fi;
        foreach (j; 0 .. f.length) {
            uint ei = m.edgeIndex(f[j], f[(j + 1) % f.length]);
            if (ei == ~0u) continue;
...
private void buildLoopEdgeFaces(ref const(Mesh) m, out uint[][] edgeFaces) {
    uint[][] ve, vf;
    m.buildLoopIncidence(ve, edgeFaces, vf, /*wantVertexStars*/ false);
}
...
private int windingFaceFor(ref const(Mesh) m, const(uint)[] incFaces, uint a, uint b) {
    foreach (fi; incFaces) {
        const f = m.faces[fi];
        foreach (j; 0 .. f.length)
...
/// face.)
private bool loopFanWalk(ref const(Mesh) m, uint b, int startFace, bool fwd, uint iters,
                         const(uint[][]) edgeFaces, out uint cand) {
    cand = ~0u;
    int face = startFace;
    foreach (_; 0 .. iters) {
        ...
        const ce = x == ~0u ? ~0u : m.edgeIndex(b, x);
...
private void selectLoopVertexWalk(ref const(Mesh) m, uint a0, uint b0,
                                  bool[] eMark, bool[] resultV,
                                  const(uint[][]) vertEdges,
                                  const(uint[][]) edgeFaces) {
    uint a = a0, b = b0;
    while (true) {
        uint curE = m.edgeIndex(a, b);
        if (curE == ~0u) break;
        ...
        if (angleGate) {
            ...
            const d0 = m.edges[cand][0] == b ? m.edges[cand][1] : m.edges[cand][0];
            const va = m.vertices[a] - m.vertices[b];
            const vc = m.vertices[d0] - m.vertices[b];
        ...
        if (eMark[cand]) break;
        eMark[cand] = true;
        const d = m.edges[cand][0] == b ? m.edges[cand][1] : m.edges[cand][0];
        resultV[b] = true;
        resultV[d] = true;
        a = b; b = d;
    }
...
bool[] selectLoopVertices(ref const(Mesh) m) {
    bool[] resultV = new bool[](m.vertices.length);
    bool[] vMark   = new bool[](m.vertices.length); // consumed as seed/in a loop
    bool[] eMark   = new bool[](m.edges.length);    // walked/consumed edges
    ...
    foreach (i; 0 .. m.vertices.length)
        if (i < m.vertexMarks.length && (m.vertexMarks[i] & Mesh.Marks.Select) != 0)
    ...
    selVerts.sort!((x, y) {
        int ox = vOrderOf(m.vertexSelectionOrder, x), oy = vOrderOf(m.vertexSelectionOrder, y);
    ...
    m.buildLoopIncidence(vertEdges, edgeFaces, vertFaces);
    ...
    SeedBurn[] burns;
    size_t[] burnOf = new size_t[](m.vertices.length); // vertex → burn index
    burnOf[] = size_t.max;
    ...
    while (true) {
        ...
        m.selectLoopVertexWalk(m.edges[seedE][0], m.edges[seedE][1], eMark, resultV,
                             vertEdges, edgeFaces);
        m.selectLoopVertexWalk(m.edges[seedE][1], m.edges[seedE][0], eMark, resultV,
                             vertEdges, edgeFaces);
    }
    return resultV;
}
...
version (unittest) size_t gSelectLoopSeedScanSteps;
...
private void selectBandTrace(ref const(Mesh) m, uint vB0, uint vA0, ubyte[] mark, bool[] resultF,
                             const(uint[][]) edgeFaces) {
    uint vB = vB0, vA = vA0;
    while (true) {
        uint ei = m.edgeIndex(vA, vB);
        if (ei == ~0u) break;
        bool advanced = false, stop = false;
        foreach (q; edgeFaces[ei]) {
            const qf = m.faces[q];
            immutable mLen = qf.length;
            if (mLen <= 2 || (mLen & 1) != 0) continue;     // degenerate / odd-sided skip
            uint j = ~0u;
            foreach (jj; 0 .. mLen)
                if (qf[jj] == vB && qf[(jj + 1) % mLen] == vA) { j = cast(uint)jj; break; }
            if (j == ~0u) continue;                    // directed-consecutive miss
            if ((mark[q] & 1) != 0) { stop = true; break; } // visited → closure stop
            mark[q] |= 1;
            resultF[q] = true;
            uint nvA = qf[(j + mLen / 2) % mLen];
            uint nvB = qf[(j + mLen / 2 + 1) % mLen];
            vA = nvA; vB = nvB;
            advanced = true;
            break;
        }
...
/// before calling).
private int findLoopPartner(ref const(Mesh) m, uint A, const(ubyte[]) mark,
                            const(uint[][]) edgeFaces,
                            const(size_t[]) partnerRank,
                            out size_t pi, out size_t pj) {
    const af = m.faces[A];
...
bool[] selectLoopFaces(ref const(Mesh) m) {
    bool[] resultF = new bool[](m.faces.length);
    // bit0 = visited (in a band result this invocation, commit filter);
    // bit1 = seeded (consumed as a group seed).
    ubyte[] mark = new ubyte[](m.faces.length);
    ...
    foreach (i; 0 .. m.faces.length)
        if (i < m.faceMarks.length && (m.faceMarks[i] & Mesh.Marks.Select) != 0)
    ...
    import std.algorithm.sorting : sort;
    selFaces.sort!((x, y) {
        int ox = fOrderOf(m.faceSelectionOrder, x), oy = fOrderOf(m.faceSelectionOrder, y);
        return ox != oy ? ox < oy : x < y;
    });
    ...
    uint[][] edgeFaces;
    m.buildLoopEdgeFaces(edgeFaces);
    ...
    size_t[] partnerRank = new size_t[](m.faces.length);
    partnerRank[] = size_t.max;
    {
        size_t rank = 0;
        foreach (fi; selFaces) {
            immutable L = m.faces[fi].length;
    ...
    while (true) {
        ...
        for (; gCur < selFaces.length; ++gCur) {
            ...
            if (m.faces[fi].length <= 2) continue;
        ...
        size_t pi, pj;
        const int P = m.findLoopPartner(A, mark, edgeFaces, partnerRank, pi, pj);
        ...
        if (P >= 0) {
            ...
            m.selectBandTrace(vB0, vA0, mark, resultF, edgeFaces); // B-side from P
            m.selectBandTrace(vB1, vA1, mark, resultF, edgeFaces); // A-side from A
        } else {
            immutable half = n / 2;
            vB0 = af[1];                vA0 = af[0];         // edge 0, reversed
            vB1 = af[(half + 1) % n];   vA1 = af[half % n];  // edge n/2, reversed
            m.selectBandTrace(vB0, vA0, mark, resultF, edgeFaces);
            m.selectBandTrace(vB1, vA1, mark, resultF, edgeFaces);
        }
    }
    return resultF;
}
...
version (unittest) {
    bool[] selectLoopFacesHeadRestart(ref const(Mesh) m) {
        bool[] resultF = new bool[](m.faces.length);
        ubyte[] mark   = new ubyte[](m.faces.length);
    ...
    bool[] selectLoopVerticesHeadRestart(ref const(Mesh) m) {
        bool[] resultV = new bool[](m.vertices.length);
        bool[] vMark   = new bool[](m.vertices.length);
        bool[] eMark   = new bool[](m.edges.length);
...
// A member of `Mesh` BEATS a same-name UFCS free function silently — no
// ambiguity, no warning — so anything that puts one of these names back on
// the struct (`mixin MeshSelectLoopOps;`, `mixin ...!();`, a named mixin, a
// hand-written method with the old body) rebinds every call site to it and
// this module becomes dead code. The regex census in
// tests/unit/commit_seam_census_test.d sees only the literal spelling; this
// sees the fact, and at `dub build` time, not only under --config=tests.
// Every later track-1 stage adds one line of names here or in its own module.
// ---------------------------------------------------------------------------
static foreach (n; ["isEdgeBorder", "isVertexBorder", "borderEdgeCountAtVertex",
                    "selectLoopEdges", "selectLoopVertices", "selectLoopFaces",
                    "selectLoopBorderChain", "selectLoopDirectionEdges",
                    "selectLoopFacesHeadRestart", "selectLoopVerticesHeadRestart"])
    static assert(!__traits(hasMember, Mesh, n),
      ...
      ~ "Stage C means nothing. Whatever re-added it — `mixin MeshSelectLoopOps;`, "
