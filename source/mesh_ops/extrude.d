...
// ---------------------------------------------------------------------------
// The EXTRUDE / EXTEND family — five kernels (`extrudeEdgesByMask` / Edge
// Extrude, `extrudeVerticesByMask` / Vertex Extrude (Cone), `extendEdgesByMask`
// / Edge Extend, `extrudeFacesByMask` / Face Extrude, `smoothShiftFacesByMask`
// / Smooth Shift) plus the family's declared edit class `kExtrudeEditScope`.
//
// MODULE-LEVEL FREE FUNCTIONS over the edit seam since task 1903 Stage H (was
// `mixin template MeshExtrudeOps`, mixed into `struct Mesh`). Split out of
// mesh.d before that by the mesh.d decomposition campaign (0407 §B.V2 — task
// 0412's doc records why a mixin template was chosen over a package move or
// free functions at the time; 1903 §4.1 is why that answer changed). This is
// the LAST family in the conversion track — task 1903 Stage I lands in the
// SAME commit and takes `struct Mesh`'s mixin count from 1 to 0.
//
// ONE RECEIVER CLASS across all five, because every one of them mutates:
// `ref MeshEditBatch ed`. Bodies are otherwise VERBATIM — every implicit
// `this` reference below is now `ed.`; the two nested TYPES of `Mesh` this
// file reaches unqualified in the old body (`Marks`, `VertSub`) are
// `Mesh.`-qualified now, since a free function cannot see a struct's nested
// type unqualified; and the two `static` members this file reaches
// (`Mesh.faceAttrOr` ×10, `Mesh.rebuildFaceWithVertexSubs` ×1, both widened at
// earlier stages — plan §2.6) keep their `Mesh.` qualifier because UFCS
// through the batch handle cannot reach a `static` member. `finalizeTopologyEdit`
// (×3, also §2.6, widened at Stage E4) is an ordinary INSTANCE method, so its
// three call sites just gained the `ed.` prefix like everything else.
//
// THE TRACKER TRAFFIC — THE ONLY FILE IN THE WHOLE FAMILY THAT HAS ANY, AND
// THE REASON THIS STAGE'S GATE IS AN OP-LOG IDENTITY MEASUREMENT, NOT ONLY A
// GEOMETRY DIFFERENTIAL. Two of the five kernels, `extrudeEdgesByMask` and
// `extendEdgesByMask`, back `mesh.edgeExtrude` and `mesh.edgeExtend` — two of
// the FOUR commands whose delta-undo is DEFAULT-ON in production
// (`CLAUDE.md`'s Undo/redo paragraph: `delete`, `remove`, `edge_extend`,
// `edge_extrude`). Their nine `editRecorder_.record*` calls and two
// `editRecorder_ !is null` guards are now `ed.rec().recordXxx(...)` and
// `ed.recording()` — `MeshEditBatch.rec()` / `.recording()` read exactly the
// frame `editRecorder_` used to be (plan §2.6's row for `Mesh.editRecorder_`:
// "NOT published. Routed through `MeshEditBatch.recording()` / `.rec()`.
// Every spelling inside `mesh.d` is unchanged"), so every downstream spelling
// — `if (recExtrude) …`, `if (recExtend) …` — is untouched. The recorded
// op-log this produces is measured BYTE-IDENTICAL, kinds and payload bytes,
// before and after this conversion (task 1903 Stage H card, "op-log
// identity") — that measurement, not the geometry differential, is what a
// reviewer of this stage should read first.
//
// §5.7's SECOND production raw position write (the first was
// `mesh_ops/decimate.d`, Stage D2). `extrudeVerticesByMask`'s apex-shift line
// — formerly `vertices[vi] = vertices[vi] + n * (shift + width);` at
// `extrude.d:2341` — is now `ed.setVertexPos(vi, …)`, bit-exact:
// `setVertexPos`'s no-op predicate is BIT identity, not `==`, so it writes
// (and stamps) whenever the raw assignment would have, and `vi` is visited
// once per call — the enclosing loop is gated on `accepted[vi]` — so there is
// no repeated-index hazard for the collect-then-write shape `setVertexPositions`
// would need (памятка 30 in the task card). The four raw writes that remain in
// this file are `m.vertices = […]` unittest fixtures below the kernels —
// `kAllow` entries, the same reason `box_geom.d`'s and `loop_slice.d`'s
// survivor carry: a local fixture mesh has no batch open around it and needs
// none.
// ---------------------------------------------------------------------------
...
enum uint kExtrudeEditScope = MeshEditScope.Geometry | MeshEditScope.Marks;
...
size_t extrudeEdgesByMask(ref MeshEditBatch ed, in bool[] maskIn, float extrude, float width) {
    ...
    const bool recExtrude = ed.recording();
    ...
    if (recExtrude) {
        foreach (i; 0 .. ed.edges.length) {
            if (i < ed.edgeMarks.length && (ed.edgeMarks[i] & Mesh.Marks.Select)) {
                preEdgeSelEnds ~= ed.edges[i][0];
                preEdgeSelEnds ~= ed.edges[i][1];
            }
        }
    }
    ...
    foreach (i; 0 .. ed.edges.length) {
        if (!mask[i]) continue;
        uint va = ed.edges[i][0], vb = ed.edges[i][1];
        ...
        bool coplanar = false;
        if (fB == -1) {
            ne = ed.faceNormal(cast(uint)fA);                 // boundary edge
        } else {
            Vec3 nA = ed.faceNormal(cast(uint)fA);
            Vec3 nB = ed.faceNormal(cast(uint)fB);
            Vec3 sum = nA + nB;
            if (sum.length < 1e-6f)
                ne = ed.faceNormal(cast(uint)(fA < fB ? fA : fB)); // opposed → lower-index fallback
            else
                ne = normalize(sum);
    ...
    Vec3 cornerNormalAt(uint v, int fi) {
        ...
        foreach (k; 0 .. f.length) {
            ...
            Vec3 cn = cross(ed.vertices[next] - ed.vertices[v], ed.vertices[prev] - ed.vertices[v]);
    ...
    foreach (v, acc; ridgeAccum) {
        ...
        ridgeVert[v] = ed.addVertex(ed.vertices[v] + dir * extrude);
    }
    ...
    Vec3 inwardDir(uint va, uint vb, int fi) {
        Vec3 t  = normalize(ed.vertices[vb] - ed.vertices[va]);
        Vec3 nf = ed.faceNormal(cast(uint)fi);
        Vec3 d  = cross(nf, t);
        ...
        auto f = ed.faces[fi];
        Vec3 c = Vec3(0, 0, 0);
        foreach (vid; f) c = c + ed.vertices[vid];
        c = c * (1.0f / cast(float)f.length);
        Vec3 mid = (ed.vertices[va] + ed.vertices[vb]) * 0.5f;
    ...
    Vec3 boundaryEdgeDir(uint v, uint other, int fi, out float farLen, out uint farVert) {
        ...
        foreach (k; 0 .. f.length) {
            ...
            Vec3 d = ed.vertices[far] - ed.vertices[v];
            if (d.length < 1e-6f) return Vec3(0, 0, 0);
            farLen = d.length;
            farVert = far;
            return normalize(d);
    ...
    foreach (i; 0 .. ed.edges.length)
        if (mask[i]) selEdgeKeys[edgeKey(ed.edges[i][0], ed.edges[i][1])] = true;
    ...
    bool capMiterInset(uint v, int fi, out Vec3 pos, out uint farVert, out float farLen) {
        ...
        foreach (k; 0 .. f.length) {
            ...
            uint prev = f[(k + f.length - 1) % f.length];
            uint next = f[(k + 1) % f.length];
            Vec3 e1 = ed.vertices[prev] - ed.vertices[v];
            Vec3 e2 = ed.vertices[next] - ed.vertices[v];
            ...
            pos = ed.vertices[v] + bis * (width / s);
            if (f.length == 4) {
                farVert = f[(k + 2) % 4];
                farLen = (ed.vertices[farVert] - ed.vertices[v]).length;
            }
            return true;
        }
        return false;
    ...
    foreach (k, acc; insetAccum) {
        ...
        if (isFreeEnd(farVertId)) {
            ...
            insetVert[k] = mv;
            insetPosWeld[weldKeyOf(v, (ed.vertices[v] + ed.vertices[farVertId]) * 0.5f)] = mv;
            weldedToFar[k] = true;
            continue;
        }
        Vec3 p = ed.vertices[v] + normalize(acc) * (*cap);
    ...
    foreach (k, farVertId; capMiterFarVert) {
        ...
        float offLen = (insetPosOverride[k] - ed.vertices[v]).length;
        ...
        if (auto farOverride = farKey in insetPosOverride) {
            ...
            float farOffLen = (*farOverride - ed.vertices[farVertId]).length;
            if (offLen + farOffLen < farLen - 1e-6f) continue;   // not yet meeting
            anyOvershootSaturated = true;
            Vec3 meetPos = ed.faceCentroid(fi);
            ...
            insetPosWeld[weldKeyOf(v, ed.vertices[farVertId])] = farVertId;
            weldedToFar[k] = true;
        }
    }
    ...
    foreach (k, acc; insetAccum) {
        ...
        //     offset entirely.
        Vec3 p = (k in insetPosOverride) ? insetPosOverride[k] : ed.vertices[v] + off;
        ...
        foreach (fi; 0 .. ed.faces.length) {
            ...
            foreach (k; 0 .. f.length) {
                ...
                foreach (far; [prev, next]) {
                    ...
                    Vec3 dir = ed.vertices[far] - ed.vertices[v];
                    if (dir.length < 1e-6f) continue;
                    chamferEdgeInset[ek] = ed.addVertex(ed.vertices[v] + normalize(dir) * width);
                }
            }
        }
    }
    ...
    if (recExtrude) {
        foreach (fi, _; affectedFaces) {
            ...
            nbrReshapeBefore ~= ed.faces[fi].dup;
        }
    }
    foreach (fi, _; affectedFaces) {
        auto f = ed.faces[fi];
    ...
    if (recExtrude && nbrReshapeIdx.length) {
        ...
        foreach (fi; nbrReshapeIdx) nbrReshapeAfter ~= ed.faces[fi].dup;
        ed.rec().recordReshapeFaces(nbrReshapeIdx, nbrReshapeBefore, nbrReshapeAfter);
    }
    ...
    bool[uint] needsAlong;
    {
        ...
        foreach (v, _; needsAlong) {
            ...
            if (auto fp = v in alongFar) {
                t = ed.vertices[*fp] - ed.vertices[v];
                ...
                t = ed.vertices[v] - ed.vertices[*op];   // fallback: extruded tangent
            } else continue;
            if (t.length < 1e-6f) continue;
            t = normalize(t);
            freeEndAlongVert[v] = ed.addVertex(ed.vertices[v] + t * offLen);
        }
    }
    ...
    foreach (fi; ed.faceIndices) {
        ...
        if (touched) {
            ...
            if (dot(ed.faceNormal(cast(uint)fi), origNormal) < 0.0f) {
                auto r = ed.faces[fi].dup;
                foreach (j, vid; r) ed.faces[fi][r.length - 1 - j] = vid;
            }
            sideTouched[cast(uint)fi] = true;
            if (recExtrude) {
                sideReshapeIdx    ~= fi;
                sideReshapeBefore ~= f;            // loop-top dup = pre-rewrite list
                sideReshapeAfter  ~= ed.faces[fi].dup; // post-rewrite (incl. flip)
            }
        }
    }
    if (recExtrude && sideReshapeIdx.length)
        ed.rec().recordReshapeFaces(sideReshapeIdx, sideReshapeBefore, sideReshapeAfter);
    ...
    const firstBridge = ed.faceAppendBase();   // task 0831: the AddFaces tail base, read PRE-append
    uint[] bridgeMaterialSrc;   // neighbor face id each bridge inherits from
    void emitBridge(uint[4] corners, Vec3 ne, int srcFace) {
        uint bfi = cast(uint)ed.faces.length;
        ed.faces ~= [corners[0], corners[1], corners[2], corners[3]];
        if (dot(ed.faceNormal(bfi), ne) < 0.0f) {
            // reverse to make the bridge consistently wound
            ed.faces[bfi] = [corners[3], corners[2], corners[1], corners[0]];
        }
    ...
    void emitBridgeFromFace(uint iA, uint iB, uint ridgeA, uint ridgeB,
        ...
        uint bfi = cast(uint)ed.faces.length;
        if (fiAtoB) ed.faces ~= [iB, iA, ridgeA, ridgeB];
        else        ed.faces ~= [iA, iB, ridgeB, ridgeA];
        bridgeMaterialSrc ~= cast(uint)srcFace;
    }
    ...
    void emitCap(uint[3] corners, Vec3 outward, int srcFace) {
        uint cfi = cast(uint)ed.faces.length;
        ed.faces ~= [corners[0], corners[1], corners[2]];
        if (dot(ed.faceNormal(cfi), outward) < 0.0f)
            ed.faces[cfi] = [corners[2], corners[1], corners[0]];
        bridgeMaterialSrc ~= cast(uint)srcFace;
    }
    ...
    foreach (ref e; exEdges) {
        ...
        if (e.fB != -1) {
            ...
            int brA = cast(int)ed.faces.length;
            ...
            int brB = cast(int)ed.faces.length;
            ...
            if (faAtoB)
                ed.faces ~= [iB, iA, e.va, e.vb];
            else
                ed.faces ~= [iA, iB, e.vb, e.va];
            bridgeMaterialSrc ~= cast(uint)e.fA;
            if (faAtoB)
                ed.faces ~= [e.vb, e.va, *rpa, *rpb];
            else
                ed.faces ~= [e.va, e.vb, *rpb, *rpa];
            bridgeMaterialSrc ~= cast(uint)e.fA;
        }
    }
    ...
    foreach (bi; 0 .. ed.faces.length - firstBridge) {
        uint srcFace = bridgeMaterialSrc[bi];
        ed.faceMaterial       ~= Mesh.faceAttrOr(ed.faceMaterial, srcFace);
        ed.facePart           ~= Mesh.faceAttrOr(ed.facePart, srcFace);
        ed.faceSetMask        ~= Mesh.faceAttrOr(ed.faceSetMask, srcFace);   // task 1060, Stage 5c
        ed.faceSelectionOrder ~= 0;
    }
    ed.resizeSubpatch();
    ...
    foreach (bi; 0 .. ed.faces.length - firstBridge) {
        ...
        ed.setFaceSubpatch(fi, ed.isFaceSubpatch(bridgeMaterialSrc[bi]));
    }
    ...
    if (recExtrude && ed.faces.length > firstBridge) {
        uint[][] bridgeLists;
        foreach (fi; firstBridge.raw .. ed.faces.length) bridgeLists ~= ed.faces[fi].dup;
        ed.rec().recordAddFaces(firstBridge, cast(uint)ed.faces.length, bridgeLists);
    }
    ...
    foreach (ref e; exEdges) {
        if (e.fB == -1 && isChamferEnd(e.va) && isChamferEnd(e.vb)) {
            ...
            ridgeEdgePos ~= [ed.vertices[ta], ed.vertices[tb]];
        } else {
            ...
            ridgeEdgePos ~= [ed.vertices[*ra], ed.vertices[*rb]];
        }
    }
    ...
    size_t facesLenBeforeCleanup = ed.faces.length;
    int[] faceRemap = new int[](facesLenBeforeCleanup);
    {
        bool[] dropFace = new bool[](ed.faces.length);
        bool anyDrop = false;
        ...
        foreach (fi; ed.faceIndices) {
            ...
            if (ed.faces[fi].length < 3) { dropFace[fi] = true; anyDrop = true; }
        }
        if (recExtrude && reduceReshapeIdx.length)
            ed.rec().recordReshapeFaces(reduceReshapeIdx, reduceReshapeBefore, reduceReshapeAfter);
        if (anyDrop) {
            uint[][] keptFaces;
            ...
            keptFaces.reserve(ed.faces.length);
            ...
            foreach (fi; ed.faceIndices) {
                ...
                if (dropFace[fi]) {
                    ...
                    if (recExtrude) {
                        ...
                        droppedFaceMat   ~= Mesh.faceAttrOr(ed.faceMaterial, fi);
                        droppedFacePart  ~= Mesh.faceAttrOr(ed.facePart, fi);
                        droppedFaceSub   ~= (ed.isFaceSubpatch(fi) ? 1u : 0u);
                        droppedFaceSetMask ~= Mesh.faceAttrOr(ed.faceSetMask, fi);
                        droppedFaceOrd   ~= Mesh.faceAttrOr(ed.faceSelectionOrder, fi);
                    }
                    continue;
                }
    ...
    if (anyOvershootSaturated) {
        ...
        foreach (fi, st; state) {
            ...
            if (recExtrude) {
                ...
                windReshapeBefore ~= r;
                windReshapeAfter  ~= ed.faces[fi].dup;
            }
        }
        if (recExtrude && windReshapeIdx.length)
            ed.rec().recordReshapeFaces(windReshapeIdx, windReshapeBefore, windReshapeAfter);
    }
    ...
    ed.finalizeTopologyEdit();   // compactUnreferenced remaps verts; rebuilds edges + edgeIndexMap
    ed.resizeVertexSelection();
    ed.resizeFaceSelection();
    ed.clearEdgeSelectionResize();   // resize edge marks + drop all edge selection
    ...
    if (recExtrude) {
        ...
        foreach (i; 0 .. ed.edges.length) {
            if (i < ed.edgeMarks.length && (ed.edgeMarks[i] & Mesh.Marks.Select)) {
                postEdgeSelEnds ~= ed.edges[i][0];
                postEdgeSelEnds ~= ed.edges[i][1];
            }
        }
        ed.rec().recordEdgeSelByEnds(preEdgeSelEnds, postEdgeSelEnds);
    }
...
size_t extrudeVerticesByMask(ref MeshEditBatch ed, in bool[] maskIn, float shift, float width)
{
    const mask = ed.maskMinusHiddenVertices(maskIn);  // §3.3 backstop (task 0613) — see maskMinusHidden* in mesh.d
    if (mask.length != ed.vertices.length) return 0;
    ...
    auto edgeFacesMap = ed.buildEdgeFaces();

    bool[] accepted = new bool[](ed.vertices.length);
    size_t processed = 0;
    foreach (vi; 0 .. cast(uint)ed.vertices.length) {
        if (vi >= mask.length || !mask[vi]) continue;

        uint[] incEdges;
        foreach (ei; ed.edgesAroundVertex(vi)) incEdges ~= ei;
        if (incEdges.length < 3) continue;

        bool manifold = true;
        foreach (ei; incEdges) {
            ulong key = edgeKey(ed.edges[ei][0], ed.edges[ei][1]);
    ...
    const uint origVertCount = cast(uint)ed.vertices.length;
    const uint origFaceCount = cast(uint)ed.faces.length;
    ...
    foreach (vi; 0 .. origVertCount) {
        ...
        Vec3 vpos = ed.vertices[vi];
        foreach (ei; ed.edgesAroundVertex(vi)) {
            uint other = ed.edgeOtherVertex(cast(uint)ei, vi);
            Vec3 sp = vpos + width * safeNormalize(ed.vertices[other] - vpos);
            ulong k = (cast(ulong)vi << 32) | cast(uint)ei;
            rimOf[k] = ed.addVertex(sp);
            fanOf[k] = ed.addVertex(sp);
        }
    }
    ...
    if (shift != 0.0f) {
        foreach (vi; 0 .. origVertCount) {
            if (!accepted[vi]) continue;
            Vec3 n = Vec3(0, 0, 0);
            foreach (fi; ed.facesAroundVertex(vi)) n = n + ed.faceNormal(cast(uint)fi);
            float len = n.length;
            n = (len > 1e-6f) ? n * (1.0f / len) : Vec3(0, 1, 0);
            ed.setVertexPos(vi, ed.vertices[vi] + n * (shift + width));
        }
    }
    ...
    foreach (vi; 0 .. origVertCount) {
        if (!accepted[vi]) continue;
        foreach (fi; ed.facesAroundVertex(vi)) {
            uint p = predInFace_(cast(uint)fi, vi);
            uint s = succInFace_(cast(uint)fi, vi);
            uint peIdx = ed.edgeIndexMap[edgeKey(p, vi)];
            uint seIdx = ed.edgeIndexMap[edgeKey(vi, s)];
            ...
            faceSubs.require(cast(uint)fi) ~= Mesh.VertSub(vi, [rimPred, rimSucc]);
    ...
    foreach (fi; 0 .. origFaceCount) {
        ...
        newFaces ~= Mesh.rebuildFaceWithVertexSubs(orig, fi in faceSubs);
        oldOfNew ~= fi;
    }
    ...
    foreach (i; cast(size_t) origFaceCount .. ed.faces.length)
        ed.faceSelectionOrder[i] = 0;
...
size_t extendEdgesByMask(ref MeshEditBatch ed, in bool[] maskIn,
    ...
    const bool recExtend = ed.recording();
    uint[] preEdgeSelEnds;
    if (recExtend) {
        foreach (i; 0 .. ed.edges.length) {
            if (i < ed.edgeMarks.length && (ed.edgeMarks[i] & Mesh.Marks.Select)) {
    ...
    Vec3 boundaryOutwardPerp(uint va, uint vb, int fi) {
        ...
        auto f = ed.faces[fi];
        Vec3 ctr = Vec3(0, 0, 0);
        foreach (vid; f) ctr = ctr + ed.vertices[vid];
    ...
    foreach (ref e; exEdges) {
        ...
        float g = dot(ed.faceNormal(cast(uint)e.fA), ed.faceNormal(cast(uint)e.fB));
    ...
    foreach (fi; 0 .. ed.faces.length)
        if (!axisAligned(ed.faceNormal(cast(uint)fi))) { meshAxisAligned = false; break; }
    ...
    int orientFaceOf(ref ExtEdge e) {
        ...
        auto ka = orientKey(ed.faceNormal(cast(uint)e.fA));
        auto kb = orientKey(ed.faceNormal(cast(uint)e.fB));
        foreach (i; 0 .. 3) {
    ...
    foreach (ref e; exEdges) {
        ...
        foreach (k; 1 .. N + 1) {
            ...
            ed.faces ~= [innerA, outerA, outerB, innerB];
        }
    }
    ...
    foreach (bi, ref e; exEdges) {
        int orientFace = orientFaceOf(e);
        uint mat  = Mesh.faceAttrOr(ed.faceMaterial, orientFace);
        uint part = Mesh.faceAttrOr(ed.facePart, orientFace);
        ulong setm = Mesh.faceAttrOr(ed.faceSetMask, orientFace);   // task 1060, Stage 5c
        foreach (k; 1 .. N + 1) {
            ed.faceMaterial       ~= mat;
            ed.facePart           ~= part;
            ed.faceSetMask        ~= setm;
    ...
    // [firstBridge .. $).
    {
        ...
        foreach (bi, ref e; exEdges) {
            int orientFace = orientFaceOf(e);
            bool sub = ed.isFaceSubpatch(orientFace);
            foreach (k; 1 .. N + 1) {
                ed.setFaceSubpatch(cursor, sub);
                ++cursor;
            }
        }
    }
    ...
    if (recExtend && ed.faces.length > firstBridge) {
        uint[][] bridgeLists;
        foreach (fi; firstBridge.raw .. ed.faces.length) bridgeLists ~= ed.faces[fi].dup;
        ed.rec().recordAddFaces(firstBridge,
                                     cast(uint)ed.faces.length, bridgeLists);
    }
...
size_t extrudeFacesByMask(ref MeshEditBatch ed, in bool[] maskIn, float distance, bool smooth = false,
    ...
    foreach (fi; 0 .. ed.faces.length)
        if (mask[fi]) normSum = normSum + ed.faceNormal(cast(uint)fi);
    {
    ...
    if (smooth) {
        ...
        foreach (fi; 0 .. ed.faces.length) {
            ...
            if (ringStartCornerSign(ed.vertices, ed.faces[fi]) < 0) fn = fn * -1.0f;
    ...
    foreach (fi; 0 .. ed.faces.length) {
        ...
        foreach (vid; ed.faces[fi]) {
            ulong k = ivKey(island, vid);
            if (k !in vertMap) {
                immutable uint nv = ed.addVertex(ed.vertices[vid] + vertOffset[k]);
                vertMap[k] = nv;
                if (remapUv) {
                    PolyVertexBlend b;
                    b.add(vid, 1.0f);
    ...
    ed.finalizeTopologyEdit();   // compactUnreferenced removes original selected-face verts not kept by walls
...
size_t smoothShiftFacesByMask(ref MeshEditBatch ed, in bool[] maskIn, float shift, float scale, bool thicken) {
    const mask = ed.maskMinusHiddenFaces(maskIn);  // §3.3 backstop (task 0613) — see maskMinusHidden* in mesh.d
    if (mask.length != ed.faces.length) return 0;
    size_t selCount = 0;
    foreach (b; mask) if (b) ++selCount;
    ...
    Vec3[int] islandCentroid;
    {
        ...
        foreach (fi; 0 .. ed.faces.length) {
            ...
            foreach (vid; ed.faces[fi]) {
                ...
                if (p is null) { sum[island] = ed.vertices[vid]; cnt[island] = 1; }
                else           { *p = *p + ed.vertices[vid]; cnt[island] = cnt[island] + 1; }
            }
        }
        foreach (isl, s; sum)
...
// ---------------------------------------------------------------------------
// TRIPWIRE — none of the family's five names may be a MEMBER of `Mesh` again.
//
// A member BEATS a same-name UFCS free function, so re-adding one (by hand, by
// a reinstated `mixin MeshExtrudeOps;`, or by an in-struct `alias`) would
// silently rebind every call site to the member and leave the free function
// unreachable while every extrude/extend test, every frozen fixture and the
// op-log identity measurement stayed green — the failure plan Revision 2
// caveat 1 describes, and `commit_seam_census_test.d`'s mixin-count row
// cannot see on its own, because it reads mesh.d's text for `mixin Mesh*Ops`
// and a hand-written method is not one (task 1903 §4.5, MINOR-1 from the
// Stage G review — that review found this exact block ABSENT from every one
// of the eight earlier tripwires' own census coverage, "deleting the whole
// `hasMember` block was green on both lanes"; this file's census row is what
// closes that for all five names at once).
//
// An in-struct `alias extrudeEdgesByMask = mesh_ops.extrude.extrudeEdgesByMask;`
// counts: MEASURED at E2 and repeated at E3/E4/F1/F2/G, an alias makes both
...
// ---------------------------------------------------------------------------
static foreach (n; ["extrudeEdgesByMask", "extrudeVerticesByMask",
                    "extendEdgesByMask", "extrudeFacesByMask",
                    "smoothShiftFacesByMask"])
    static assert(!__traits(hasMember, Mesh, n),
        "`Mesh." ~ n ~ "` is a MEMBER again. Task 1903 Stage H moved this "
      ~ "family out of `struct Mesh` into module-level free functions over "
      ~ "`ref MeshEditBatch` in mesh_ops/extrude.d. A member BEATS a "
      ~ "same-name UFCS free function, so every call site — five commands, "
      ~ "five tools (commit and, for the three PreviewRebuild tools, preview), "
      ~ "the Topology Pen's duplicate-edges gesture, and several hundred "
      ~ "unittest sites — would silently bind back to the member and the free "
      ~ "function would go unreachable while all of them stayed green. An "
      ~ "in-struct `alias` counts: it makes `hasMember` answer true, which is "
      ~ "why plan §2.7 forbids it (task 1903 §2.7, §4.5).");
...
// The `extendEdgesByMask` kernel-call helper for this file's own module
// unittests (F1's `sliceOnce` shape, F2's `bevelOnce`; the same lesson as
// памятка 46 -- build a call through a factory, not a bare foreach -- applied
// to a single call site rather than a corpus). It is the ONE thing every
// direct-call unittest below needed once the kernel stopped taking a bare
// `Mesh`: open an UNRECORDED batch (these are geometry/topology fixtures, not
// op-log fixtures -- recording one here would build a delta no assertion
// reads), call the kernel, close, return its count. Declared here rather than
// at module scope alongside the kernels because it is `version (unittest)`
// only, like the selection helpers above it.
version (unittest) private size_t extendOnce(ref Mesh m, in bool[] mask, float inset,
                                             float shift, Vec3 offset, Vec3 rotateDeg,
                                             Vec3 scale, int segments,
                                             Vec3 pivot = Vec3(0, 0, 0)) {
    auto ed = MeshEditBatch.unrecorded(m, kExtrudeEditScope);
    immutable n = ed.extendEdgesByMask(mask, inset, shift, offset, rotateDeg, scale,
                                       segments, pivot);
    ed.close();
    return n;
}
...
unittest { // extendEdgesByMask: cube interior edge — identity, offset, rotate, scale, combined
    ...
    // ---- identity (inset=0.1, shift=0.2, no TRS) → 10v/7f, bridge [6,8,9,7] ----
    {
        ...
        auto n = extendOnce(m, selMask_(m), 0.1f, 0.2f,
    ...
    // ---- offset=(0,0.3,0) → new verts (±0.4, 0.7, 0.4) ----
    {
        Mesh m = makeCube();
        selectEdgeByEnds_(m, Vec3(-0.5f, 0.5f, 0.5f), Vec3(0.5f, 0.5f, 0.5f));
        extendOnce(m, selMask_(m), 0.1f, 0.2f,
...
unittest { // extendEdgesByMask: pivot arg — rotZ=30 about a NONZERO pivot equals
    ...
    extendOnce(m0, selMask_(m0), 0.1f, 0.2f,
                         Vec3(0, 0, 0), Vec3(0, 0, 30.0f), Vec3(1, 1, 1),
                         1, /*pivot=*/Vec3(0, 0, 0));
...
unittest { // extendEdgesByMask: shift inert on interior edge (inset=0, shift=0.4)
    Mesh m = makeCube();
    selectEdgeByEnds_(m, Vec3(-0.5f, 0.5f, 0.5f), Vec3(0.5f, 0.5f, 0.5f));
    auto n = extendOnce(m, selMask_(m), 0.0f, 0.4f,
                                 Vec3(0, 0, 0), Vec3(0, 0, 0), Vec3(1, 1, 1), 1);
    assert(n == 1);
...
unittest { // extendEdgesByMask: chain2 weld (two top edges sharing corner (0.5,0.5,0.5))
    ...
    auto n = extendOnce(m, selMask_(m), 0.1f, 0.0f,
                                 Vec3(0, 0, 0), Vec3(0, 0, 0), Vec3(1, 1, 1), 1);
    assert(n == 2);
...
unittest { // extendEdgesByMask: COLLINEAR boundary chain — shared mid-rim vertex
    ...
    auto n = extendOnce(m, selMask_(m), 0.0f, 0.2f,
                                 Vec3(0, 0, 0), Vec3(0, 0, 0), Vec3(1, 1, 1), 1);
    assert(n == 2);
...
unittest { // extendEdgesByMask: CLOSED-RING boundary probe (task 0477 P11 REV1
    ...
    auto n = extendOnce(m, mask, 0.0f, 0.0f,
                                 Vec3(0, 0, 0), Vec3(0, 0, 0), Vec3(1, 1, 1), 1);
...
unittest { // extendEdgesByMask seg3 offY=0.3 — ring Y = 0.4 + k/3·0.3 (0.5/0.6/0.7)
    Mesh m = makeCube();
    selectEdgeByEnds_(m, Vec3(-0.5f, 0.5f, 0.5f), Vec3(0.5f, 0.5f, 0.5f));
    extendOnce(m, selMask_(m), 0.1f, 0.0f,
