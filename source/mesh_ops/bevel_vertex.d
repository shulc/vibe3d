...
// ---------------------------------------------------------------------------
// `bevelVerticesByMask` (vertex bevel / vertex chamfer) and the one corner
// record its umbrella walk is built out of.
//
...
// (commands/mesh/vertex_bevel.d, plus tools/edit/vertex_bevel_tool.d).
//
...
//
// Converted from `mixin template MeshBevelVertexOps` to module-level FREE
// FUNCTIONS by task 1903 Stage E4 (`doc/mesh_edit_seam_plan.md` §4, §5.2 row
// E4, which pairs this file with `mesh_ops/bevel_fin.d`). The function BODY is
// unchanged — every edit is an `ed.` prefix, plus the four spellings a module
// scope forces (`Mesh.VertSub`, `Mesh.Marks`, `Mesh.rebuildFaceWithVertexSubs`
// and `rewriteFaces(ed, …)` in place of the implicit `this`).
//
// ONE RECEIVER, `ref MeshEditBatch ed`, because the single entry mutates
// (§4.1 cell one). Cells two and three do not occur: there is no read-only
// entry here and nothing memoizes.
//
// §4.4a's FOURTH CELL does NOT occur, and that is measured rather than
// assumed: `source/mesh.d` and the remaining `mesh_ops/*.d` were grepped for
// `bevelVerticesByMask` and `VertexBevelCorner` before the conversion, and
// every hit outside this file is inside a COMMENT (mesh.d's `CornerDrop`
// banner and the old `mixin` site; three cross-reference remarks in
// edge_bevel.d and extrude.d). The two production callers are OUTSIDE the
// mesh — `commands/mesh/vertex_bevel.d` and `tools/edit/vertex_bevel_tool.d`
// — and both open the batch at their own boundary, which is the finished
// shape and not a debt.
//
...
//     `setFaceMarksFrom(faceMarks, ~Marks.Select)` over the whole word. NOT
//     an ordering defect: moving that clear in front of `rewriteFaces` was
//     tried, and the loss list is byte-identical — the rewrite then carries
//     the already-cleared word and the `FaceReindex` reverse restores it
//     faithfully. Only DELETING the clear recovers the plane, and that
//     changes the shipped law. What this plane needs is a MARKS publisher
//     (plan L0's first production publishers), not a reorder.
//   * `vertexMarks`, `vertexSelectionOrder`, `vertexSelectionOrderCounter` —
//     the tail `clearVertexSelection()`; dropping that call recovers exactly
//     these three. Not face planes: no `FaceReindex` could ever carry them.
//   * `edgeMarks`, `edgeSetMask`, `vertexSetMask` — lost in every variant
//     measured; the edge-array rebuild and the set resize in
//     `finalizeTopologyEdit`. Not face planes either.
//
// `faceSelectionOrder[i] = 0` over the cap range is NOT a cause: it writes
// only APPENDED slots, which have no pre-op value, and `faceSelectionOrder`
// survives the armed revert in every variant. So the earlier reading of this
// as "two carried planes written after the rewrite" was wrong on both
// halves, and the remedy L7 owes is a marks publisher (L0) for the face
...
// because this file stopped being a mixin body instantiated in mesh.d's scope.
// The census rows naming their callers are in
// tests/unit/commit_seam_census_test.d.
// ---------------------------------------------------------------------------

/// The edit class the vertex chamfer declares, in ONE place. It adds vertices
/// and faces (`Points | Polygons`) and rewrites the face selection (`Marks`);
/// it moves no EXISTING vertex, which is why `Position` — the bit decimate and
/// the plane cut carry — is absent. Callers pass this to `MeshEditBatch`; the
/// kernel's own `commitChange` keeps its literal spelling, exactly as cut.d
/// and cleanup.d do.
enum uint kBevelVertexEditScope = MeshEditScope.Geometry | MeshEditScope.Marks;
...
///
/// MODULE SCOPE, not a member of `Mesh` (plan §2.7 as rewritten at the E2
/// review). The mixin injected it into the struct; nothing outside this file
/// ever spelled `Mesh.VertexBevelCorner`, so unlike `RevolveParams` /
/// `PlaneCutLoops` this move had no call sites to carry — but the rule is the
/// same one, and so is the refusal of an in-struct `alias`: an alias makes
/// `__traits(hasMember, Mesh, "VertexBevelCorner")` answer `true` and the
/// tripwire at the foot of this file cannot both stand and be answerable.
struct VertexBevelCorner { uint fi, pred, succ; }
...
size_t bevelVerticesByMask(ref MeshEditBatch ed, const bool[] maskIn, float amount) {
    ...
    foreach (vi; 0 .. origVertCount) {
        ...
        foreach (other; umbrella[vi]) {
            ...
            if (key !in splitByKey) {
                Vec3 sp = ed.vertices[vi] +
                          amount * safeNormalize(ed.vertices[other] - ed.vertices[vi]);
                splitByKey[key] = ed.addVertex(sp);
            }
            ring ~= splitByKey[key];
        }
        ...
        foreach (fi; ed.facesAroundVertex(cast(uint)vi)) { donor = cast(uint)fi; break; }
    ...
    foreach (vi; 0 .. origVertCount) {
        ...
        foreach (c; corners[vi]) {
            ...
            faceSubs.require(c.fi) ~= Mesh.VertSub(cast(uint)vi, [spPV, spVS]);
        }
    }
    ...
    foreach (fi; 0 .. ed.faces.length) {
        ...
        newFaces ~= Mesh.rebuildFaceWithVertexSubs(orig, cast(uint)fi in faceSubs);
    ...
    foreach (fi; capStart .. ed.faces.length)
    ...
    // the zero.
    ed.dropCornerProvenance(CornerDrop.VertexBevelNoCase);
...
// these names back on the struct (`mixin MeshBevelVertexOps;`, `mixin ...!()`,
// a named mixin, a hand-written method with the old body, or an
// `alias VertexBevelCorner = …;`) rebinds every call site to it and this module
...
// --config=tests.
//
// It is ALSO the check that the mutating receiver did not quietly widen back: a
// `Mesh.bevelVerticesByMask` member could only exist by taking the mesh
// directly, i.e. by dropping the batch this stage exists to require.
//
// BOTH names are listed, the corner record included: the mixin put it into
// `Mesh` too (plan §2.7), and an `alias VertexBevelCorner = …;` inside `Mesh`
// is exactly the fallback §2.7 used to offer and E2/E3 measured to be
// self-defeating — with an alias in place, "is it still a member of `Mesh`?"
// stops being answerable.
// ---------------------------------------------------------------------------
static foreach (n; ["bevelVerticesByMask", "VertexBevelCorner"])
    static assert(!__traits(hasMember, Mesh, n),
      ...
      ~ "kernel takes as its receiver goes away with it, and task 1903 Stage E4 "
      ~ "means nothing. Whatever re-added it — `mixin MeshBevelVertexOps;`, "
