...
// ---------------------------------------------------------------------------
// The plane-cut kernel family: `cutByPlane` / `cutByPlaneRestricted` /
// `cutByPlaneClipped` / `cutByPlaneEx` / `cutByPlaneSplitGap` /
// `deleteComponentsInSlab`, plus the private `planeCutCore`, `isConcaveFace`,
// `extractCutLoops`, `splitAlongCutLoop` and the result type `PlaneCutLoops`.
// Split out of mesh.d as `mixin template MeshCutOps` — the PILOT of the mesh.d
// decomposition campaign (0407 §B.V2, task 0412); see that task for the
// original symbol inventory.
//
// Converted to module-level FREE FUNCTIONS by task 1903 Stage E3
// (`doc/mesh_edit_seam_plan.md` §4, §5.2 row E3). Function BODIES are
// unchanged — every edit is an `ed.` / `m.` prefix — and the shapes settled at
// C / D1 / D2 / D3 / E1 / E2 hold here verbatim: the receiver of a mutating
// kernel is the batch, the `mixin MeshCutOps;` line left `struct Mesh` in the
// SAME change, and the callers open an UNRECORDED batch at their own boundary.
// What E3 adds to that memo is below.
//
// TWO RECEIVERS, AND THE SPLIT IS THE MUTATION, NOT THE `const` (§4.1 cells
// one and two):
//
//   * `ref MeshEditBatch ed` — `cutByPlane`, `cutByPlaneRestricted`,
//     `cutByPlaneClipped`, `cutByPlaneEx`, `cutByPlaneSplitGap`,
//     `deleteComponentsInSlab`, `planeCutCore`, `splitAlongCutLoop`. All eight
//     insert vertices, rewrite windings or drop faces.
//   * `ref const(Mesh) m` — `isConcaveFace` (a reflex-vertex test over one
//     winding) and `extractCutLoops` (a pure walk of `edges` against a cut
//     mask). `isConcaveFace` WAS a `const` member; `extractCutLoops` was not,
//     and nothing in a mixin body forced the keyword — so the const receiver
//     here is a widening of what the code already did, now ENFORCED at the
//     seam. Both are called from a batch context as `f(ed.mesh, …)`, the
//     spelling E1 settled for `computeCollapsedFace_`.
//
// §4.1's THIRD cell (a query that touches a `mutationVersion`-keyed memo and
// therefore needs a plain `ref Mesh`) does not occur here: nothing in this file
// memoizes.
//
// §4.4a's FOURTH cell does not occur either, and that is measured, not
...
// 3 sites in `source/tools/slice/slice_tool.d`, 7 in
// `tests/unit/mesh_ops/cut_test.d` and 1 in
// `tests/unit/tools/slice/slice_tool_test.d`, and all 11 are in this commit.
// The alternative §2.7 used to offer — an `alias PlaneCutLoops = …;` inside
// `Mesh` — is REFUSED, and the refusal is measured: an in-struct alias makes
// `Mesh.PlaneCutLoops` resolve again AND makes
// `__traits(hasMember, Mesh, "PlaneCutLoops")` answer `true`, which kills the
// tripwire at the foot of this file. A `public import` alone cannot bring
// `Mesh.X` back, so there is no third shape.
//
// THE §2.6 WIDENINGS THIS STAGE PAYS FOR (three, all `private` members of
// `mesh.d` that a mixin body reached for free): `Mesh.edgeIndexOfVerts` (3
// sites here), `Mesh.insertEdgePoint` (1) and `Mesh.rebuildFacesWithChordSplits`
// (1). Each has a census row in `tests/unit/commit_seam_census_test.d` naming
// this file as its caller — the row reddens both when the name narrows back
// and when a SECOND file starts leaning on the open door.
//
// THE RAW POSITION WRITE THIS FAMILY OWNED, AND THE PREDICATE THAT COULD NOT
// SEE IT (§5.7): `splitAlongCutLoop`'s gap block moved the two members of each
// seam pair with `vertices[pr[0]] = …` — a NESTED index expression, which
// §5.7's predicate (`vertices\[[^\]]*\]`) refuses to match. So this file read
// 0 raw writes under a predicate that was structurally blind to the only two
// it had. Both are `ed.setVertexPositions` now, the predicate admits one level
// of index nesting, and those two lines are its positive controls. Re-measured
// under the widened predicate the tree gains 9 more visible sites, none of them
// E3's: `mesh_ops/loop_slice.d` ×4 (Stage F1) and `commands/mesh/{magnet,
// vertex_center,vertex_set}.d` ×3 — all three already named in §5.7's L0 list.
// ---------------------------------------------------------------------------
...
/// the op-log's declared scope when Stage L4 turns this family's undo into a
/// delta (`MeshEditTracker.declare` is what ends up in `MeshEditDelta.scope_`).
///
/// `Geometry` = Points|Polygons: every cut inserts crossing vertices, rewrites
/// windings, and the split-gap path deletes the band component outright.
/// `Marks`: the chord rebuild copies selection / subpatch / material words onto
/// both halves and `syncSelection` resizes the mark planes. `Position`: the
/// Gap option MOVES existing seam vertices — this is the second family in the
/// tree to carry that bit (decimate is the first), and the reason is the
/// `ed.setVertexPositions` call in `splitAlongCutLoop`.
enum uint kCutEditScope =
...
size_t cutByPlane(ref MeshEditBatch ed, Vec3 p, Vec3 n, float eps = 1e-5f) {
    bool[] cv;
    Vec3[] edir;
    return planeCutCore(ed, p, n, /*clipped*/false, Vec3(0, 0, 0), Vec3(0, 0, 0),
                        /*restrictFaces*/null, cv, eps, edir);
}
...
size_t cutByPlaneRestricted(ref MeshEditBatch ed, Vec3 p, Vec3 n,
                            const uint[] restrictFaces, float eps = 1e-5f) {
    bool[] cv;
    Vec3[] edir;
    return planeCutCore(ed, p, n, /*clipped*/false, Vec3(0, 0, 0), Vec3(0, 0, 0),
                        restrictFaces, cv, eps, edir);
}
...
// -----------------------------------------------------------------------
private bool isConcaveFace(ref const(Mesh) m, uint fi) {
    ...
    foreach (i; 0 .. face.length) {
        Vec3 prev = m.vertices[face[(i + face.length - 1) % face.length]];
        Vec3 cur  = m.vertices[face[i]];
        Vec3 next = m.vertices[face[(i + 1) % face.length]];
...
// -----------------------------------------------------------------------
private size_t planeCutCore(ref MeshEditBatch ed, Vec3 p, Vec3 n, bool clipped,
    ...
    isCutVertOut = null;
    cutEdgeDirOut = null;
    if (ed.vertices.length == 0 || ed.faces.length == 0 || ed.edges.length == 0)
        return 0;
    ...
    if (restricted) {
        faceInRestrict.length = ed.faces.length;
        vertInRestrict.length = ed.vertices.length;
        edgeInRestrict.length = ed.edges.length;
        foreach (rf; restrictFaces) {
            if (rf >= ed.faces.length) continue;
            faceInRestrict[rf] = true;
            auto face = ed.faces[rf];
            foreach (k; 0 .. face.length) {
                ...
                uint ei = ed.edgeIndexOfVerts(a, b);
    ...
    float[] dv;
    dv.length = ed.vertices.length;
    bool[] onPlane;
    onPlane.length = ed.vertices.length;
    foreach (vi; 0 .. ed.vertices.length) {
        Vec3 v = ed.vertices[vi];
    ...
    Vec3 crossPoint(uint a, uint b) {
        ...
        float t = da / (da - db);
        Vec3 va = ed.vertices[a], vb = ed.vertices[b];
        return Vec3(va.x + t * (vb.x - va.x),
                    va.y + t * (vb.y - va.y),
                    va.z + t * (vb.z - va.z));
    }
    bool bandOkVert(uint a)          { return !clipped || inBand(ed.vertices[a]); }
    ...
    if (clipped) {
        edgeTouchesConcaveFace.length = ed.edges.length;
        foreach (fi; 0 .. ed.faces.length) {
            if (!isConcaveFace(ed.mesh, cast(uint)fi)) continue;
            auto cface = ed.faces[fi];
            foreach (k; 0 .. cface.length) {
                uint a = cface[k], b = cface[(k + 1) % cface.length];
                uint cei = ed.edgeIndexOfVerts(a, b);
            ...
            foreach (k; 0 .. face.length) {
                ...
                if ((da > 0 && db < 0) || (da < 0 && db > 0)) {
                    ...
                    cxS[nCx] = ((q.x - segStart.x) * seg.x
                              ...
                              + (q.z - segStart.z) * seg.z) / segLen2;
                    cxEdge[nCx] = ed.edgeIndexOfVerts(a, b);
                    nCx++;
                }
            }
    ...
    bool[] isCutVert;
    isCutVert.length = ed.vertices.length;
    foreach (vi; 0 .. ed.vertices.length)
    ...
    Vec3[uint] edgeDirOf;
    size_t origEdgeCount = ed.edges.length;
    foreach (ei; 0 .. origEdgeCount) {
        uint a = ed.edges[ei][0], b = ed.edges[ei][1];
        ...
        uint vi = ed.insertEdgePoint(cast(uint)ei, da / (da - db), isCutVert);
        ...
        edgeDirOf[vi] = normalize(ed.vertices[posEnd] - ed.vertices[negEnd]);
    }
    ...
    if (termini.length) {
        ...
        foreach (rec; termini) {
            if (rec.faceIdx >= ed.faces.length) continue;
            auto face = ed.faces[rec.faceIdx];
            ...
            uint bVert = face[bk];
            uint tVert = ed.addVertex(rec.point);
            if (isCutVert.length < ed.vertices.length)
                isCutVert.length = ed.vertices.length;     // grow; T stays non-cut (false)
            ed.faces[rec.faceIdx] = face[0 .. bk + 1] ~ [tVert, bVert] ~ face[bk + 1 .. $];
    ...
    bool[] effMask;
    if (termini.length) {
        effMask.length = ed.faces.length;
        foreach (fi; 0 .. ed.faces.length)
            effMask[fi] = (!restricted || faceInRestrict[fi]) && !termFace[fi];
    } else if (restricted) {
        effMask = faceInRestrict;
    }
    size_t nSplit = ed.rebuildFacesWithChordSplits(effMask, isCutVert);
    ...
    cutEdgeDirOut.length = ed.vertices.length;
...
// -----------------------------------------------------------------------
size_t cutByPlaneClipped(ref MeshEditBatch ed, Vec3 p, Vec3 n,
                         Vec3 segStart, Vec3 segEnd,
                         float eps = 1e-5f, const uint[] restrictFaces = null) {
    bool[] cv;
    Vec3[] edir;
    return planeCutCore(ed, p, n, /*clipped*/true, segStart, segEnd,
                        restrictFaces, cv, eps, edir);
}
...
size_t cutByPlaneEx(ref MeshEditBatch ed, Vec3 p, Vec3 n, bool clipped,
                    Vec3 segStart, Vec3 segEnd,
    ...
    bool[] isCutVert;
    Vec3[] cutEdgeDir;
    size_t nSplit = planeCutCore(ed, p, n, clipped, segStart, segEnd,
    ...
    result.loops = extractCutLoops(ed.mesh, isCutVert);
    ...
    if (split)
        splitAlongCutLoop(ed, isCutVert, p, n, caps, result.seamPairs, eps,
...
// -----------------------------------------------------------------------
size_t deleteComponentsInSlab(ref MeshEditBatch ed, Vec3 p, Vec3 n,
                              float loSigned, float hiSigned, float eps) {
    if (ed.faces.length == 0) return 0;

    int[] parent;
    parent.length = ed.vertices.length;
    foreach (i; 0 .. ed.vertices.length) parent[i] = cast(int)i;
    int findRoot(int x) {
    ...
    foreach (fi; 0 .. ed.faces.length) {
        ...
        foreach (i; 1 .. f.length) {
            if (f[0] >= ed.vertices.length || f[i] >= ed.vertices.length) continue;
    ...
    foreach (vi; 0 .. ed.vertices.length) {
        ...
        float dv = n.x * (ed.vertices[vi].x - p.x)
                 + n.y * (ed.vertices[vi].y - p.y)
                 + n.z * (ed.vertices[vi].z - p.z);
    ...
    bool[] mask = new bool[](ed.faces.length);
    size_t nMasked = 0;
    foreach (fi; 0 .. ed.faces.length) {
        if (ed.faces[fi].length == 0) continue;
        uint v0 = ed.faces[fi][0];
        if (v0 >= ed.vertices.length) continue;
    ...
    if (nMasked == ed.faces.length) return 0;
...
// -----------------------------------------------------------------------
size_t cutByPlaneSplitGap(ref MeshEditBatch ed, Vec3 p, Vec3 n, bool clipped,
                          Vec3 segStart, Vec3 segEnd,
    ...
    PlaneCutLoops r1, r2;
    size_t n1 = cutByPlaneEx(ed, p + n * loAmt, n, clipped, segStart, segEnd,
                             /*split*/true, caps, r1, eps, restrictFaces, 0.0f, 0);
    size_t n2 = cutByPlaneEx(ed, p - n * hiAmt, n, clipped, segStart, segEnd,
                             /*split*/true, caps, r2, eps, restrictFaces, 0.0f, 0);
    size_t removed = deleteComponentsInSlab(ed, p, n, -hiAmt, +loAmt, 1e-4f);
    separated = (removed > 0);
    return n1 + n2;
}
...
// -----------------------------------------------------------------------
private uint[][] extractCutLoops(ref const(Mesh) m, const bool[] isCutVert) {
...
// -----------------------------------------------------------------------
private void splitAlongCutLoop(ref MeshEditBatch ed, const bool[] isCutVert,
                               Vec3 planeP, Vec3 planeN,
    ...
    uint getDup(uint vi) {
        ...
        uint nv = ed.addVertex(ed.vertices[vi]);
        dupOf[vi] = nv;
    ...
    foreach (fi; 0 .. ed.faces.length) {
        ...
        foreach (v; ed.faces[fi]) {
            if (cut(v)) continue;
            float d = planeN.x * (ed.vertices[v].x - planeP.x)
                    + planeN.y * (ed.vertices[v].y - planeP.y)
                    + planeN.z * (ed.vertices[v].z - planeP.z);
            if      (d >  eps) ++pos;
            else if (d < -eps) ++neg;
        }
    ...
    if (gap != 0.0f) {
        ...
        // THE ONE RECORDED WRITE IN THIS FAMILY (task 1903 §2.5, §5.7). Until
        // Stage E3 these two lines were `vertices[pr[0]] = …` — a RAW
        // coordinate write, invisible to every mutation hook, so a recording
        // batch around a split-gap cut produced an op-log that named the added
        // vertices and the reshaped faces and said NOTHING about the seam pair
        // it had just pushed apart. `§5.7`'s own predicate could not see them
        // either: it was written `vertices\[[^\]]*\]`, which refuses to match a
        // NESTED index expression, and `pr[0]` is one (measured at E3 — the
        // predicate now admits one level of nesting and carries these two lines
        ...
        // vertex, and its skip predicate is BIT identity, not `==`, so a
        // zero-amount side (`gapSide` positive/negative) still flips a `-0.0`
        // coordinate to `+0.0` exactly as the raw `v + dir*0` did. Each seam
        // vertex belongs to exactly one pair (`getDup` dedups the originals and
        // mints the duplicates), so no index is written twice and gathering
        // before writing cannot change what any single write sees.
        uint[] seamMoveIdx;
        Vec3[] seamMoveTo;
        seamMoveIdx.reserve(seamPairs.length * 2);
        seamMoveTo.reserve(seamPairs.length * 2);
        foreach (pr; seamPairs) {
            ...
            seamMoveIdx ~= pr[0];
            seamMoveTo  ~= ed.vertices[pr[0]] + dir * loAmt;   // lo → toward +n endpoint
            seamMoveIdx ~= pr[1];
            seamMoveTo  ~= ed.vertices[pr[1]] - dir * hiAmt;   // hi → toward −n endpoint
        }
...
unittest { // cutByPlaneSplitGap: axis-cube two-cut COMPOSITION gate (task 0291,
    ...
    size_t n;
    {
        ...
        n = ed.cutByPlaneSplitGap(P, N, /*clipped*/false, P, P,
...
// these names back on the struct (`mixin MeshCutOps;`, `mixin ...!()`, a named
// mixin, a hand-written method with the old body, or an
// `alias PlaneCutLoops = …;`) rebinds every call site to it and this module
...
// `Mesh.cutByPlaneEx` member could only exist by taking the mesh directly, i.e.
// by dropping the batch this stage exists to require.
//
// EVERY family name is listed, including the private helpers and the injected
// result struct: the mixin put all of them into `Mesh` (plan §2.7), so a
// partial revert that reinstates only `PlaneCutLoops` — the one an outside
// caller spells — is exactly the silent half this list is here to catch. That
// entry is also the one that refuses the `alias PlaneCutLoops = …;` inside
// `Mesh` that §2.7 used to offer as a fallback: measured, an in-struct alias
// makes `__traits(hasMember, Mesh, "PlaneCutLoops")` answer `true`, so the
// alias and this assert cannot both stand.
// ---------------------------------------------------------------------------
static foreach (n; ["cutByPlane", "cutByPlaneRestricted", "isConcaveFace",
                    "planeCutCore", "cutByPlaneClipped", "PlaneCutLoops",
                    "cutByPlaneEx", "deleteComponentsInSlab",
                    "cutByPlaneSplitGap", "extractCutLoops",
                    "splitAlongCutLoop"])
    static assert(!__traits(hasMember, Mesh, n),
      ...
      ~ "Stage E3 means nothing — including the recorded `setVertexPositions` "
      ~ "write in splitAlongCutLoop, which has no batch to record into once the "
      ~ "receiver is the mesh. Whatever re-added it — `mixin MeshCutOps;`, "
      ~ "`mixin ...!()`, a named mixin, an `alias`, or a hand-written method — "
      ~ "must go.");
