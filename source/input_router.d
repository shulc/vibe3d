...
struct InputRouter {
    ...
    // The held mouse buttons (slice M1a, `source/held_gesture_buttons.d`):
    // while any is held no key press or release is dispatched. Set for every
    // press this router delivers, cleared per button at the top of
    // `processEvent`, wholesale on focus loss and by the automation reset.
    alias held_ = g_heldGestureButtons;
    ...
    bool processEvent(SDL_Event* ev, bool eventWindowFocused) {
        ...
        // FIRST, before the pie grab, ImGui and the viewport swallow gate: any
