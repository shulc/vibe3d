...
scratch=$(mktemp -d "${TMPDIR:-/var/tmp}/vibe3d-web-editor.XXXXXX")
server_pid=
cleanup() {
...
for mode in normal spreset; do
    ...
    timeout 110 node "$repo_root/tools/web_editor_liveness.mjs" \
        "$chromium" "http://127.0.0.1:$port/?probe=w16-editor-$mode-liveness" \
        "$scratch/profile-$mode" "${VIBE3D_WEB_EDITOR_DELAY_MS:-12000}"
    echo "WEB-EDITOR mode=$mode delayed-input=live"
done
cp "$scratch/vibe3d-normal.js" "$repo_root/.build/web-editor/vibe3d.js"
