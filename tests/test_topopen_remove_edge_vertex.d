...
///
/// DISARM FIRST, and it is not defensive decoration: `/api/reset` does NOT drop
/// the active tool, and `toolHost.activate()` deactivates the outgoing one —
/// which for a session tool means COMMITTING it. The runner shares one app
/// across a worker's whole slice, so "whatever the previous test left" is not a
/// baseline this file may lean on.
CameraState armCube() {
