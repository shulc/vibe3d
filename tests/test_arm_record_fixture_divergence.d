...
unittest {
 ...
 toolBaseline();auto a0=lifecycleCount();cmd("tool.set move on");auto a=evidencePoint("arm",lifecycleCount()==a0+1?"new":"none");auto d0=lifecycleCount();cmd("tool.set move off");auto d=evidencePoint("drop",lifecycleCount()!=d0?"lifecycle":"none");postJson("/api/command", commandBody("history.undo"));settle();auto u=evidencePoint("undo");u["move_family_active"]=(tool()=="move");retirement(law(fx,"arm_owns_record"),JSONValue([a,d,u]));
 toolBaseline();auto ra0=lifecycleCount();cmd("tool.set move on");auto ra=evidencePoint("arm",lifecycleCount()==ra0+1?"new":"none");postJson("/api/command", commandBody("history.undo"));settle();auto rau=evidencePoint("undo");postJson("/api/command", commandBody("history.redo"));settle();auto rar=evidencePoint("redo");rar["move_family_active"]=(tool()=="move");retirement(law(fx,"move_arm_undo_redo"),JSONValue([ra,rau,rar]));
 toolBaseline();auto rk0=lifecycleCount();cmd("tool.set mesh.sliceTool on");auto rk=evidencePoint("arm",lifecycleCount()==rk0+1?"new":"none");postJson("/api/command", commandBody("history.undo"));settle();auto rku=evidencePoint("undo");postJson("/api/command", commandBody("history.redo"));settle();auto rkr=evidencePoint("redo");rkr["cutting_family_active"]=(tool()=="cutting");retirement(law(fx,"cutting_arm_undo_redo"),JSONValue([rk,rku,rkr]));
 toolBaseline();cmd("tool.set move on");auto m=point("move",tool());cmd("tool.set mesh.sliceTool on");auto k=point("cutting",tool());auto swapUndo=postJson("/api/command", commandBody("history.undo"));assert(swapUndo["status"].str=="ok",swapUndo.toString);settle();auto su=point("undo",tool());retirement(law(fx,"swap_undo_previous_family"),JSONValue([m,k,su]));
 auto ru=point("undo",tool());auto rr=postJson("/api/command", commandBody("history.redo"));settle();auto rpj=point("redo",tool());rpj["result"]=(rr["status"].str=="ok"?"accepted":"unavailable");retirement(law(fx,"swap_redo_unavailable"),JSONValue([ru,rpj]));
...
unittest {
 ...
 postJson("/api/command", commandBody("history.undo"));settle();obs~=walkPoint("undo_b",geomEq(vertices(),g1)?"g1":geomEq(vertices(),g0)?"g0":"unexpected");postJson("/api/command", commandBody("history.undo"));settle();obs~=walkPoint("undo_edit",geomEq(vertices(),g0)?"g0":geomEq(vertices(),g1)?"g1":"unexpected");postJson("/api/command", commandBody("history.redo"));settle();obs~=walkPoint("redo_edit",geomEq(vertices(),g1)?"g1":"unexpected");postJson("/api/command", commandBody("history.redo"));settle();obs~=walkPoint("redo_b",geomEq(vertices(),g1)?"g1":"unexpected");retirementFull(law(fx,"viewport_selection_undo_redo"),JSONValue(obs));
}
...
unittest {
 ...
 postJson("/api/command", commandBody("history.undo"));settle();obs~=walkPoint("undo_b",geomEq(vertices(),g1)?"g1":geomEq(vertices(),g0)?"g0":"unexpected");
 postJson("/api/command", commandBody("history.undo"));settle();obs~=walkPoint("undo_edit",geomEq(vertices(),g0)?"g0":geomEq(vertices(),g1)?"g1":"unexpected");
 postJson("/api/command", commandBody("history.redo"));settle();obs~=walkPoint("redo_edit",geomEq(vertices(),g1)?"g1":"unexpected");
