...
unittest {
 ...
 toolBaseline();auto ra0=lifecycleCount();cmd("tool.set move on");auto ra=evidencePoint("arm",lifecycleCount()==ra0+1?"new":"none");postJson("/api/command", commandBody("history.undo"));settle();auto rau=evidencePoint("undo");postJson("/api/redo","");settle();auto rar=evidencePoint("redo");rar["move_family_active"]=(tool()=="move");retirement(law(fx,"move_arm_undo_redo"),JSONValue([ra,rau,rar]));
 toolBaseline();auto rk0=lifecycleCount();cmd("tool.set mesh.sliceTool on");auto rk=evidencePoint("arm",lifecycleCount()==rk0+1?"new":"none");postJson("/api/command", commandBody("history.undo"));settle();auto rku=evidencePoint("undo");postJson("/api/redo","");settle();auto rkr=evidencePoint("redo");rkr["cutting_family_active"]=(tool()=="cutting");retirement(law(fx,"cutting_arm_undo_redo"),JSONValue([rk,rku,rkr]));
...
unittest {
 ...
 postJson("/api/command", commandBody("history.undo"));settle();obs~=walkPoint("undo_b",geomEq(vertices(),g1)?"g1":geomEq(vertices(),g0)?"g0":"unexpected");postJson("/api/command", commandBody("history.undo"));settle();obs~=walkPoint("undo_edit",geomEq(vertices(),g0)?"g0":geomEq(vertices(),g1)?"g1":"unexpected");postJson("/api/redo","");settle();obs~=walkPoint("redo_edit",geomEq(vertices(),g1)?"g1":"unexpected");postJson("/api/redo","");settle();obs~=walkPoint("redo_b",geomEq(vertices(),g1)?"g1":"unexpected");retirementFull(law(fx,"viewport_selection_undo_redo"),JSONValue(obs));
}
