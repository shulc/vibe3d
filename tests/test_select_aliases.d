// Tests for phase 4.5: select.* alias / shim commands.
//
...
// edge → polygon (ALL rule): face is selected only when every one of
...
unittest { // convert edge→poly with ONLY ONE edge → no face selected (ALL rule)
    ...
    assertSet(selectedFaces(), [],
        "convert edge→poly with single edge: no face should be selected under ALL rule");
}
