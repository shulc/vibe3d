...
unittest { // 6511: FrameProbe service is the second pre-probe owner boundary
    import std.file : dirEntries, SpanMode;
    import std.regex : matchAll, regex;

    immutable appPath = buildPath(repoRoot, "source", "app.d");
    immutable serverPath = buildPath(repoRoot, "source", "http_server.d");
    immutable app = blankNonCode(readText(appPath));
    enum serviceCounts = "httpServer.tickFrameCounts(g_fc);";
    enum serviceFrames = "httpServer.tickFrames(g_frames);";
    enum timing = "g_frames.beginFrame();";
    enum allocation = "g_fc.beginFrame();";
    assert(app.count(serviceFrames) == 1,
        "6511 owner boundary requires exactly one frame-probe call in app.d");
    immutable serviceCountsAt = app.indexOf(serviceCounts);
    ...
    foreach (entry; dirEntries(buildPath(repoRoot, "source"), "*.d",
        ...
        size_t hits;
        foreach (_; code.matchAll(regex(`\btickFrames\b`))) ++hits;
        allHits += hits;
        if (entry.name == appPath) appHits = hits;
        if (entry.name == serverPath) serverHits = hits;
    }
    assert(files >= 500, "6511 source census population fell below 500 modules");
    assert(allHits == 2,
        "6511 tickFrames identifier count must be 2 in source/**");
    assert(appHits == 1 && serverHits == 1,
        "6511 tickFrames must have one app call and one server definition");
}
...
unittest { // 6357: frame-count service is the unique pre-probe owner boundary
    ...
    assert(prefix == "if (httpServer.running) httpServer.tickFrameCounts(g_fc); "
        ...
        "6357 frame owner services must be the statements directly before the timing probe begins");
