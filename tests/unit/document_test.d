...
mixin template DocumentTests() {
    unittest { // task 4061: derived primary is memoised, never stored
        ...
        assert(doc.layers.length == 8,
            "population floor: the memo pin requires exactly 8 layers");

        // Chosen multi-select order: 3 first, then 1, then 6. The oldest
        // CURRENT seat is the edit target, independent of layers[] order.
        doc.selectItem(doc.layers[3], SelMode.Set);
        doc.selectItem(doc.layers[1], SelMode.Add);
        doc.selectItem(doc.layers[6], SelMode.Add);
        auto layer3 = doc.layers[3];
        assert(doc.primary is layer3,
            "multi-select order must resolve primary from deselection history");

        // Exercise the real layer lifecycle command and its exact undo. The
        // delete removes the walk's head by membership; undo reinserts the
        // same object and restores the causal selection/history snapshot.
        auto view = new View(0, 0, 800, 600);
        auto del = new LayerDelete(doc.activeMesh(), view, EditMode.Vertices,
                                   &doc, null);
        del.setIndex(3);
        assert(del.apply(), "layer delete in the primary memo pin must apply");
        assert(doc.layers.length == 7,
            "population check: layer delete must remove exactly one layer");
        assert(doc.primary is doc.layers[1],
            "after deleting the head, the next selected seat becomes primary");

        assert(del.revert(), "undo of the layer delete must succeed");
        assert(doc.layers.length == 8,
            "population check: undo must restore the eighth layer");
        assert(doc.primary is layer3,
            "undo must restore the primary through selection/history state");

        // This assertion is the selectItem-invalidation mutation witness.
        auto switched = doc.layers[5];
        doc.selectItem(switched, SelMode.Set);
        assert(doc.primary is switched,
            "primary after a layer switch must follow deselection history");

        // Re-establish a multi-selection in a known order before the frame-
        // shaped read window. Adds never promote: `switched` stays the head.
        doc.selectItem(doc.layers[2], SelMode.Add);
        doc.selectItem(doc.layers[7], SelMode.Add);
        assert(doc.primary is switched,
            "multi-select adds must preserve the current queue head");
        ...
        assert(accesses == 30,
            "population floor: the frame-shaped window must perform 30 reads");

        auto perf = parseJSON(g_perf.toJson());
        assert("editTargetDerive" in perf,
            "population floor: tests configuration must enable PerfProbe");
        auto raw = perf["editTargetDerive"]["count"];
        immutable long derives = raw.type == JSONType.integer
            ? raw.integer : cast(long) raw.uinteger;
        assert(derives <= 1,
            format("editTargetDerive per frame: expected <= 1 for %d accesses, got %d",
                   accesses, derives));
    }
