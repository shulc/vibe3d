...
//     either file below, at its single history call:
//         history.record(cmd);
...
// G3 record site uses — not plan §5.1's per-family call-surface census
// (`tool_commit_seam_census_g3_test.d`), which keys on the whole multiset with
...
/// G3's two record sites, one per file — the membership plan §6 gives the group
/// after two corrections it already carries: `magnet.d` moved to G1, and
/// `bend.d` / `push.d` are `TransformTool` subclasses inside the T2 zone, out
/// of scope. Both absences are deliberate; a file appearing here that plan §6
/// does not name is a roster edit, not a discovery.
private enum string[] kG3Files = [
    "smooth_shift_tool.d", "stroke_extrude_tool.d",
];
...
//
//    (a) Block 2 asserts both G3 sites spell `record`. That is worth nothing
...
// ---------------------------------------------------------------------------
// 2. THE CELL. Each of G3's two files reaches `CommandHistory` exactly once,
//    and through `record`.
//
...
unittest {
    ...
    foreach (f; kG3Files) {
        immutable path = buildPath(repoRoot, "source", "tools", "deform", f);
        assert(exists(path),
            "G3 census: `" ~ path ~ "` does not exist. The roster names a file "
    ...
    if (totalRecord != kG3Files.length)
        bad ~= "    · TOTAL: " ~ totalRecord.to!string ~ " `history.record(` "
             ~ "site(s) over " ~ kG3Files.length.to!string ~ " files, expected "
             ~ "one each. A site added or removed changes what "
             ~ "`tests/test_tool_gesture_g3.d` is an oracle FOR, and must be "
             ~ "re-frozen deliberately";

    if (bad.length == 0) return;
    string msg = "G3 record-primitive census: " ~ bad.length.to!string
               ~ " finding(s) over `source/tools/deform/**`:\n";
    foreach (b; bad) msg ~= b ~ "\n";
    assert(false, msg);
}
