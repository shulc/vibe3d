...
void postSelect(string mode, int[] indices) {
    ...
    idxJson ~= "]";
    auto resp = post(BASE ~ "/api/command", commandBody("mesh.select", `{"mode":"` ~ mode ~ `","indices":` ~ idxJson ~ `}`));
...
JSONValue postRedo() { return parseJSON(cast(string)post(BASE ~ "/api/command", commandBody("history.redo"))); }
