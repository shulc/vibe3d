...
// actionCenter.mode=border), dragging the MOVE arrow on a partial
// selection of a deforming mesh re-orients the gizmo MID-DRAG. The
// move handle's orientation (handler.axisX/Y/Z) was overwritten every
// frame from the live select-derived basis, which flips its up-sign /
// swaps its right axis as the half-cube deforms. The screen→world
// projection of each motion delta then reads a DIFFERENT frame from
// one step to the next, so the per-step response jumps discontinuously
// (the gizmo "flips direction" without releasing LMB).
//
// The fix freezes the move handle's orientation for the duration of an
// active drag (dragAxis >= 0), so the input-projection frame is fixed
// (the reference engine's drag-start-fixed gizmo). The APPLY frame
// (runFrameR/U/F) was
// already frozen; this closes the input-projection side.
//
// What this test does:
...
//      activate xfrm.flex.
//   3. Grab the MOVE Z-arrow handle and drag it in CHUNKS — one
//      play-events call per motion step, button held down across calls
//      — sampling the moving-set centroid after each step. (A single
//      one-shot play-events drag would NOT expose the mid-drag
//      re-orientation the same way the per-step trajectory does.)
//   4. Assert the per-step centroid increment along the drag axis is
//      SMOOTH (monotonic in magnitude, no discontinuous jump). Pre-fix
//      the increment jumps ~19% at the frame the select-basis flips;
//      post-fix it varies < 6% across the whole drag.
//
// The drag axis / camera here were chosen empirically so that the
...
// Centroid (per axis) of the verts that moved away from `pre`.
double[3] movedCentroid(double[3][] pre, double[3][] now) {
...
unittest {
    ...
    // case where the select-basis is sensitive to the deformation.
    ...
    // screen→world projection switches to a more edge-on axis), measured
    // at ~19 % step-to-step. Post-fix the frame is frozen and the
    // increments vary < 6 % monotonically. Assert no step-to-step jump
    // exceeds 12 %.
    double maxJump = 0;
    size_t jumpAt = 0;
    ...
    assert(maxJump < 0.12,
        "Flex move-Z drag per-step response jumped " ~ maxJump.to!string
