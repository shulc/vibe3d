...
//     stuck/corrupted state.
//
// Both scenarios use the same non-quad-neighbour mesh as
...
unittest { // P2 -- `select` toggle mid-arm degrades gracefully and recovers.
    ...
    // Toggle OFF: the latched fallback seeds cannot collect a ring at all
    // (every candidate seed's `collectEdgeRing` returns [], same reason
    // test_loop_slice_band_arm_nonquad.d needed the band-mode arm bypass) --
    // ring mode has nothing to cut. Stays ARMED, `built_` goes false, the
    // mesh reverts to the pre-arm baseline (no crash, no stuck geometry).
    ...
    assert(st2["built"].type == JSONType.false_,
        "toggling select OFF mid-arm on an ungeometrically-collectable seed "
        ~ "set must leave built=false (nothing to cut in ring mode here)");
    auto m2 = getJson("/api/model");
    assert(m2["vertexCount"].integer == 8 && m2["faceCount"].integer == 5,
        "select=off mid-arm should revert to the 8V/5F baseline, got "
