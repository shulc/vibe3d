...
unittest { // softDrag = Move + Screen, camera-facing-only
           // (xfrm.softDrag = `trans integer 0`)
...
// The shear / twist / taper presets use `shape integer 0` (Linear).
// The FalloffStage default is Smooth, so without the explicit
// `shape: linear` in tool_presets.yaml these deform tools would
// silently use the wrong attenuation curve.
unittest { // shear pins shape=linear
    clearFalloff();
...
unittest { // twist pins shape=linear
    clearFalloff();
...
unittest { // taper pins shape=linear
    clearFalloff();
...
// (sphere around the picked component). The preset leaves AXIS
// alone, so the transform gizmo stays on the
...
// xfrm.flare is a `Flare` ToolPreset — surprisingly that's
