/// test_injected_modules_linked_src.d — the SOURCE-BACKED twin of
/// test_injected_modules_linked.d (card gate-speedup): the same check, built
/// on run_test.d's other compile line (project test-lib + the `src` helper
/// object). The top-level `import math` below is what classifies it that way
/// (isSourceBackedTest); without this twin, dropping the helper object from
/// that line stayed green, because a source-backed test that imports no
/// helper links fine without it -- minus liveness_gate.
///
...
module test_injected_modules_linked_src;

import math : Vec3;   // makes this a source-backed test; see the header
